...
- **Fibonacci / golden ratio / golden spiral / phi** — `experiments/fibonacci_spiral.py`, `fibonacci_squares.py`, `phi_scheduler.py`, `golden_survey.py`, `fold_golden_closure.py`, `fold_ladder_phi.py`, `data/fold_ladder_phi_data.json` (C2: retrace chain is 1/4 golden rungs — NOT a chain law), `data/golden_survey_data.json` (phi EXACT in cusp metric; static C0 packing has no golden structure; gap-filling does NOT lock onto golden angle), `data/epoch_0d.json`
