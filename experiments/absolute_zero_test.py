"""
ABSOLUTE ZERO TEST: viscosity -> 0 = medium frozen = blowup possible?
Fast version: N=32, fewer steps, fewer nu values.
"""
import numpy as np
...
print(f"N={N} R_init={R_INIT} dt={dt} steps={STEPS} T_max={STEPS*dt:.2f}")
print()

for nu_val in [2.0, 0.5, 0.1, 0.01, 0.0]:
    print(f"--- nu = {nu_val} ---")
    ux, uy, uz = make_poloidal(R_INIT)
    ...
    for s in range(1, STEPS+1):
        ux, uy, uz = step(ux, uy, uz, nu_val)
        if np.any(np.isnan(ux)):
            print(f"  DIVERGED at t={s*dt:.3f} (Euler blowup!)")
            diverged = True
            break
        if s % SAVE_EVERY == 0:
            m = metrics(ux, uy, uz)
            gfinal = m[4]; rfinal = m[3]

    if not diverged:
        print(f"  gn14: {g0:.4f} -> {gfinal:.4f}  r_eff: {r0:.4f} -> {rfinal:.4f}")
