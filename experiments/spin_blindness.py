...
def main():
    ...
    print("  converges iff sigma(2p-d)/p < 1, i.e. p < sigma*d/(2*sigma-1)")
    print("  (all p when sigma=1/2). Enstrophy (p=2): blind iff sigma<1/(4-d),")
    print("  = Leray's unconditional bound int Z dt <= E0/(2 nu) -- sharp.")
    print()
    print("=> Time-integrated finite-p diagnostics are blind WITHIN THE")
    print("   BAND p < sigma*d/(2*sigma-1); the band shrinks as sigma")
    print("   grows but always contains enstrophy for sigma < 1/(4-d).")
    print("   Beale-Kato-Majda (p = inf, rate 2*sigma >= 1) is the unique")
    print("   power-law gate that always diverges. The missing invariant")
