...
### 3.x  Learning & Creativity Test (T74) — learned and creativity, one rubric

- **Claim:** A test ascertaining what a learner LEARNED in a learning
  environment and whether it CREATES novel-but-valid content, on two axes
  (recognition/transfer for learned; novelty × appropriateness for creativity)
  that the human-assessment protocol in `docs/LEARNING_CREATIVITY_TEST.md`
  reuses with the same thresholds.  L1: held-out probe accuracy climbs from
  near-chance 0.125 to ≥0.90 as exemplar exposure grows 1→40/concept — a
  stored-memory k-NN router (majority over its 5 nearest stored exemplars, the
  repo's own k-NN primitive) routes at chance while the memory is sparse
  because the k-neighborhood is dominated by OTHER concepts.  L2: no
  forgetting — first-taught concepts keep ≥0.92 after every later concept is
  added.  C1: a measurable share of mid-size near-miss variations are
  simultaneously NOVEL (outside the taught core) and VALID (inside the learned
  manifold, routed to the intended concept) — never-presented new-but-right
  items (≥0.24 yield).  C2: novelty ≠ creativity — random far nulls are ~100%
  novel but 0% creative, so the joint criterion is necessary.  C3: the
  creative landscape is interior-peaked — too-close variations are valid but
  not novel, too-far are novel but not valid.
- **Evidence:** `data/learn_creativity_test_data.json` (per-seed L1 curve
  [0.12, 0.39–0.55, 0.90–0.94, …], no_forgetting_min ≥0.92, mid_creative
  ≥0.24, null novel 1.00 / creative 0.00, C3 yields peaked at the mid size).
  Pinned by `test_learn_creativity_supported`.
- **References:** `experiments/learn_creativity_test.py`;
  `docs/LEARNING_CREATIVITY_TEST.md` (design doc: engine test + the human
  protocol scored on the same axes and thresholds).
- **Honest walls (crease-worthy):** the agent is a stored-memory k-NN router
  over independent gaussian exemplars in a synthetic 2D concept space — all
  claims are MECHANISM claims about the test (the environment→agent mapping is
  measurable and reproducible), not transfer to unseen tasks or open-domain
...
| Spectral C1/C3/C4 (WEAVERS Ch. 5.1) | **Tested 2026-08-08** | C1 partial (k=12 0.53%, k=26 0.96%; k=10/2000 miss); C3a not supported (no mode near λ(7)); C3b supported (mode 12.2416 vs λ(31)=12.261, Δ=0.0197); C4 → Poisson (⟨r⟩=0.372 vs GOE 0.5307) — T19 "consistent chaos" refuted at level-spacing level. `experiments/spectral_extended.py`, `data/spectral_extended_data.json` |
