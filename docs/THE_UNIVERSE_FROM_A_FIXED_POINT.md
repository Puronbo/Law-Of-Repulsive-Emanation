...
**One-loop sign change result:** Starting near the SM one-loop FP (G*=0.838, Λ*=-1.500) and integrating toward IR using the Donà et al one-loop beta functions (type-II cutoff), the trajectory crosses from AdS to dS at k ≈ 0.116 M_Pl (1.4 × 10^18 GeV), after which it diverges. The crossing illustrates a possible qualitative mechanism for obtaining a dS IR from an AdS UV; it does not address the magnitude of Λ. Note: the one-loop FP values (G*=0.838, Λ*=-1.500) differ from the Table 1 values (G*=0.57, Λ*=-0.346) because the table uses type-Ia cutoff while the flow uses type-II. Both give AdS FP with SM matter.
