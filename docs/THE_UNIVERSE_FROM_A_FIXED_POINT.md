...
#### f(R) truncation family: e-folds computed (experiments/fr_inflation.py)

The natural hypothesis -- that higher f(R) orders extend the FP-hugging phase and lengthen inflation -- is FALSE, and we can now state why quantitatively. E-folds decompose as

    N = T_stick + T_cross
    T_stick = ln(1/delta_0)/theta_re     (FP-hugging duration)
    T_cross ~ 2.0                        (nonlinear descent of L~, EH betas)

where delta_0 is the release amplitude along the relevant direction and theta_re the real critical exponent. The descent branch toward classical gravity gives T_cross = 2.04 (lambda_tilde falls below 0.1*lambda*); the opposite branch climbs to the singular line lambda_tilde -> 0.5 and is unphysical for cosmology.

Results across all truncation orders (theta_re from Codello et al Table 4):

    n:      1      3      4      5      6      7      8
    theta:  2.382  2.711  2.864  2.527  2.414  2.507  2.407
    N(delta_0=1e-2):  4.0  3.8  3.7  3.9  4.0  3.9  4.0
    N(delta_0=1e-6):  7.9  7.2  6.9  7.5  7.8  7.6  7.8

    N required for structure formation:  50 - 60
    delta_0 required for N=60:           10^-60 (n=1) to 10^-72 (n=4)

Three conclusions:

    1. Natural releases give N = 3.7 - 7.9 at EVERY order --
       confirming the EH result (~3 e-folds) with the mechanism
       now explicit.

    2. Larger f(R) exponents SHORTEN inflation: N decreases from
       n=1 to n=4 because T_stick shrinks as theta_re grows.
       Polynomial f(R) moves e-folds in the WRONG direction.
       This mirrors the CC result (section 6.3a2): truncation
       extensions do not help.

    3. The deficit is a fine-tuning statement: N=60 needs the
       universe to sit within 10^-60 to 10^-72 of the critical
       surface in the relevant direction -- the same order of
       absurdity as the CC gap. Both quantitative failures have
       one shape: exponential sensitivity where the theory
       provides no selection principle.

       Resolution (2026-09-20): the selection principle is supplied
       by the framework's own instrument -- THE_ENTROPY_CONDITION_
       THEOREM.md:251. The matching 0/0 (N_req - N(d))/(d - d*) has
       a POSITIVE removable value RV = 1/(theta*d*) at every f(R)
       truncation, so the CMB's e-fold demand is in the Theorem 3.3
       STABLE/SELECTED regime; the W=-1 ejector lane carries the
       removable point (LANE-2, lambda_0 = 0.36952 = ridge handoff),
       while the W=0 NGFP lane caps at epsilon=1 and stays on the
       POLE side. experiments/delta0_selection.py, 4/4 gates PASS,
       ledger F19.

The resolution adopted by this framework: the FP supplies the initial scale and flatness; the e-folds are generated by an explicit inflaton sector (Higgs-condensate route [31]) rather than by the pure-gravity flow.

**2026-09-19 cross-link (no claim added).** The CMB-anchored referee check
(docs/CMB_RECONCILIATION.md, REFEREE-1/2) confirms this section at
measurement level: n_s = 0.9649 +/- 0.0042 with r < 0.036 rule out the
pure-gravity slow-roll route outright (epsilon ~ -0.87 cannot be made small),
and the same measurement forces delta_0 = 10^-60..-72 in the Q2 formula
(fr_inflation.py). Where this section records "the theory provides no
selection principle," the entropy-condition theorem
(THE_ENTROPY_CONDITION_THEOREM.md:251) is the one candidate selection
instrument the framework already contains; status INVESTIGATION.
