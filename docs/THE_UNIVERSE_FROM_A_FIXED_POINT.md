...
**One-loop sign change result:** Starting near the SM FP (G*=0.838, Λ*=-1.500) and integrating toward IR using the Donà et al one-loop beta functions, the trajectory crosses from AdS to dS at k ≈ 0.116 M_Pl (1.4 × 10^18 GeV). The sign change IS achieved at one-loop. This is the mechanism that solves the cosmological constant problem: the UV FP is anti-de Sitter, but the IR universe is de Sitter.

The trajectory diverges after the sign change (Λ grows without bound), which is a known limitation of the one-loop approximation. The full flow requires two-loop or non-perturbative treatment to achieve the correct suppression from Λ* = -1.5 to Λ_obs ≈ 10^-122.
...
If the HL-LHC or FCC-ee measures the Higgs self-couplings κ₃ (trilinear) and κ₄ (quartic) and finds they are NOT consistent with a 2-parameter curve, the framework fails. The ASSM predicts exactly 2 relevant directions (θ₁ = -1.93, θ₂ = -0.811), meaning κ₃ and κ₄ are determined by the same 2 UV parameters and must lie on a 1-dimensional curve in (κ₃, κ₄) space.
