...
5. **The CC problem (SIGNIFICANTLY RESHAPED):** The pure-gravity FP has G*×Λ* = 0.12 (de Sitter). With SM matter, the FP shifts to anti-de Sitter (Λ* < 0), fundamentally changing the suppression problem. The observed positive Λ must be generated by the RG flow from negative to positive. The suppression from |Λ*_AdS| to Λ_obs is much smaller than from Λ*_dS to Λ_obs. However, the one-loop formula is approximate; the full two-loop gravity-matter flow (Pastor-Gutiérrez et al 2023) is needed for precise FP values.
