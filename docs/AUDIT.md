...
8. **Regression coverage for the T55 series + library** — the experiments print
   ...
   solvable-theorem pattern is now the norm: 19 verdict experiments
   ...
   `balance_scale`, `balance_continual`, plus the five
   earlier probes)
   each ship a claim/verdict JSON and are pinned by
   `tests/test_solvable_theorems.py` (19 tests; full suite 209).  The
