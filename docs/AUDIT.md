...
MIGRATION superseded-but-present); **(5)** the network is real loopback TCP
but still one machine — no TLS, no cross-machine transport, consensus is
majority-honesty not BFT, total state loss needs the T8 WAL, and the bridge
still has no threshold custody, KYC/AML, or regulator.
The honest
