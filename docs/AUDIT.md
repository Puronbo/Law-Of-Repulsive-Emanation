...
own WAL is the durability floor for total loss).
...
majority-honesty not BFT, an OS crash mid-commit can tear the WAL, and the
bridge still has no threshold custody, KYC/AML, or regulator.
The honest
