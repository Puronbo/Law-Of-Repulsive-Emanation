...
* **71. Shannon entropy 0/0 (2026-08-17)** — 0*log(0) = 0/0 removable value = 0; uniform distribution achieves max H = ln(n); MI 0/0 at perfect correlation and deterministic; KL 0/0 at P=Q=0. `data/shannon_entropy_0_over_0_data.json`.
* **72. Bayes theorem 0/0 (2026-08-17)** — Posterior -> prior as P(D)->0 (0/0 removable = prior); likelihood ratio -> 1 as hypotheses converge; mixture posterior valid. `data/bayes_theorem_0_over_0_data.json`.
* **73. Lorenz attractor 0/0 (2026-08-17)** — Lyapunov exponent ~0.91 (log(1)/0 = 0/0 removable); sum of exponents = -(sigma+1+beta); Hopf bifurcation at rho_H ~ 24.74. `data/lorenz_attractor_0_over_0_data.json`.
* **74. Boltzmann entropy 0/0 (2026-08-17)** — S/ln(W) = 1 (0/0 at W=1 removable=1); 0*ln(0) = 0; mixing entropy max at x=0.5; Gibbs entropy of pure state = 0. `data/boltzmann_entropy_0_over_0_data.json`.
* **75. Zeta functional equation 0/0 (2026-08-17)** — zeta(0) = -1/2 via functional equation (0*inf = 0/0, removable = -1/2); FE verified for zeta(2), zeta(3), zeta(4); trivial zeros via sin(pi*s/2) = 0. `data/zeta_functional_eq_0_over_0_data.json`.
* **76. Wigner semicircle 0/0 (2026-08-17)** — GUE/GOE eigenvalue density follows semicircle law; rho/sqrt(4-lambda^2) = 1/(2*pi) everywhere, 0/0 at band edge, removable = 1/(2*pi); spectral rigidity N(E)/E -> 1/pi; Tracy-Widom edge scaling. `data/wigner_semicircle_0_over_0_data.json`.
* **77. Noether's theorem 0/0 (2026-08-17)** — Conserved quantities from continuous symmetries; dL/depsilon = 0/0 at epsilon=0, removable = 0; momentum conserved for free particle, energy for harmonic oscillator and pendulum. `data/noether_theorem_0_over_0_data.json`.
* **78. Spectral gap 0/0 (2026-08-17)** — Gap closes at TFIM critical point h=1; Delta(L)*L -> C (0*inf = 0/0, removable = C ~ 1.57); gap*L grows in gapped phases; critical scaling converges. `data/spectral_gap_0_over_0_data.json`.
* **79. Green's function 0/0 (2026-08-17)** — Green's functions for 1D/2D Laplacian; G(x,x) singular (0/0 removable = eigenfunction reciprocal); eigenfunction expansion converges; free-space G = -(1/(2*pi))*log(r); method of images for disk. `data/greens_function_0_over_0_data.json`.
* **80. Mobius function 0/0 (2026-08-17)** — mu(n) values correct; sum_{d|n} mu(d) = [n==1] (0/0 at n=1 removable=1); Dirichlet inverse of 1 is mu; Mertens function M(x)/x -> 0 consistent with PNT; Dirichlet series 1/zeta(s). `data/mobius_function_0_over_0_data.json`.
* **81. Saddle point 0/0 (2026-08-17)** — Gaussian saddle convergence; Stirling via Laplace method; g'(x)/(x-x*) -> g''(x*) at saddle (0/0 removable = second derivative); Watson's lemma for asymptotic integrals. `data/saddle_point_0_over_0_data.json`.
* **82. Stirling's approximation 0/0 (2026-08-17)** — n!/Stirling -> 1; correction term (ratio-1)*n -> 1/12 (0*inf = 0/0, removable = 1/12); Gamma function Stirling; Wallis product pi/2 verified. `data/stirling_approx_0_over_0_data.json`.
* **83. Logarithmic limits 0/0 (2026-08-18)** — 6 sub-experiments: log(1+x)/x removable=1, log product->1, Stirling log n!/n -> 0, H_n - ln(n) -> gamma (Euler-Mascheroni), Binet F_n/phi^n -> 1/sqrt(5), Euler reflection z*Gamma(z)*Gamma(1-z) -> 1. `data/log_limits_0_over_0_data.json`.
* **84. Combinatorics 0/0 (2026-08-18)** — 6 sub-experiments: Stirling 2nd kind S(n,k)/k^n -> 1/k!, Catalan C_n*n^(3/2)/4^n -> 1/sqrt(pi), binomial binom(n,k)/n^k -> 1/k!, Motzkin M_n*n^(3/2)/3^n converges, partition log(p(n))/sqrt(n) -> pi*sqrt(2/3), derangement D_n/n! -> 1/e. `data/combinatorics_0_over_0_data.json`.
* **85. Probability ergodic 0/0 (2026-08-18)** — 6 sub-experiments: Weak LLN S_n/n -> mu, Martingale E[X_{n+1}|F_n] - X_n = 0, Birkhoff ergodic (1/N)sum f(T^nx) -> int f dmu, conditional expectation E[X|Y] = E[X], Shannon-McMillan-Breiman -(1/n)log P -> H(X), Kolmogorov 0-1 tail events. `data/probability_0_over_0_data.json`.
* **86. Number theory sums 0/0 (2026-08-18)** — 6 sub-experiments: von Mangoldt psi(x)/x -> 1, Euler totient sum phi(k)/k -> 6/pi^2, Mertens product ln(x)*prod(1-1/p) -> 1/e^gamma, Chebyshev bias pi_{4k+3} - pi_{4k+1}, Liouville L(x)/x bounded, totient sum (1/n^2)*sum phi(k) -> 3/pi^2. `data/number_theory_sums_0_over_0_data.json`.
* **87. Convex variational 0/0 (2026-08-18)** — 6 sub-experiments: Legendre transform f*(p)=sup(px-f(x)), Fenchel-Moreau f**=f, Poincare inequality, Brachistochrone time integral, Isoperimetric 4*pi*A/L^2 -> 1, Euler-Lagrange first variation=0. `data/convex_variational_0_over_0_data.json`.
* **88. Random matrix 0/0 (2026-08-18)** — 5 sub-experiments: Circular law eigenvalue disk, Tracy-Widom GUE largest eigenvalue n^{2/3}(lmax-2) -> TW_1, Wigner semicircle GUE density, Marchenko-Pastur rectangular matrix, Sample covariance mean=1. `data/random_matrix_0_over_0_data.json`.
