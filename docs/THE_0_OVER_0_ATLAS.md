...
2. **The slope reading hover-strikes its own soft negative (≈ −0.07).**
   ...
   members and the 91 removable-value entries stand, and the open pair is
...
*All 91 removable-value entries verified computationally (85-sweep + 6 open-target/named/control — the sweep's 64th, the Landau-function calibration #76 — REMOVABLE value 1, Vanishing Rate, Landau 1903; the sixth, the Mertens product-gap complement #75 — REMOVABLE value 0, elementary, Mertens 1874; the sweep's 65th, the Crossing-a-zero calibration (corpus census #80) — REMOVABLE value 0 (j ≥ 1) / v0 (j = 0), Critical Phenomenon, and its parity law: at a mutual zero of gap j the closed-form ratio (x−1)^j·V/U either *carries the value v0 = V(1)/U(1) across* (j = 0) or *arrives at rate v0·|x−1|^j* (j ≥ 1), crossing zero iff j is odd — verified exact at 60 digits, two-sided, with certified rationals 2048/65537 and 65537/67108865 as value and rate); and the sweep's closing 66th–85th, the 20-experiment round-out (corpus census #81–#100) — the Table of Great Limits (sinc, exponential rate, continuity of e, half-cosine, Taylor third, polygon perimeter, n^(1/n), Euler–Mascheroni, factorial root, central binomial), the arithmetic-landscape probes (Dirichlet divisor-summatory, sigma-summatory, primes-in-AP, consecutive-prime ratio, partition, Li-form prime counting, squarefree density, Gauss-circle), the Newton 1/(2√2) quadratic rate, and the golden-ratio Binet collapse — all removable values, all passed; entries 64, 67, 69, 70, 73 and 74 document the anti-class — the Pólya and Mertens conjectures are FALSE, proven externally (Littlewood; Odlyzko–te Riele), the Chebyshev-walk ratio has no limit, proven unconditionally (Montgomery–Vaughan Thm 15.11: ψ − x = Ω±(x^{1/2})), the divisor/circle ratios have no limit, proven unconditionally (Hardy 1916; Voronoi; Sierpiński), and the character-walk sup-ratio has no limit in q-space, proven unconditionally (Paley 1932; Pólya–Vinogradov 1918) — each borne out by its fluctuation reading (Pólya 0.486, Mertens 0.433, Chebyshev δ band 0.51·0.61·0.71·0.69·0.68·0.75 below the √n floor — the theorem, not the range, arbitrates, divisor/circle bounded-band order-one, character-walk floor 0.577+ with creeping sup); entry 71 documents the removable side's flat-decay anchor (3/π², β_ratio = −1.20, Dirichlet 1849); entry 72 documents the bounded-box control (mod-7 character partial sums: |S| ≤ 2 at every x to 1e7, S(x)/√x → 0 at β_ratio = −0.590, Pólya–Vinogradov) — the manufactured counterpoint proving the criterion reads the ratio's decade trend alone, never the size of the raw oscillation. 690 regression tests passing.*
