...
**Status note.** This section is a *correspondence mapping*, the same genre as
the rest of this document. It claims nothing physical on either side: the
patent's model is expired prior art (public domain since 2024-04-24, see
`docs/US7284987B2_ANALYSIS.md`), and its quantitative claims do not all
reproduce (the 15 axes are exactly 6-choose-4, but the claim of equal-angle
arcsin(⅓) separation is not reproducible from the stated coordinate
construction). The mapping below is interpretive, and per the audit's own
doctrine it is **not** to be cited as verified physics.

### 11.1 Narrative Element: The Centerpoint

McGrath's "Physical Quantum Model for the Atom" (US7284987B2, granted 2007)
posits a single confined spherical centerpoint in which six dimension-wave
generators reflect. Any four of the six waves converge to a 4-D lattice set,
and the 6-choose-4 metric yields **15 axes** (5 axial triplets = 5 light-cone
pairs). Quantum loops are traced along these axes on spindle-torus geometry;
mass-particles are three loops each, regularized by discrete high-density
lattice-circle radii (electron 85, quark 325, proton 1105 waves); gravity is
the background disturbance of the loops; the photon is excess resonant energy
released from loop nodes. The Model claims to be deterministic, background
independent, and to generate the mass hierarchy from available lattice sets.

### 11.2 Physical Mapping

| Patent structure | Physical reading | Mathematical content that *does* reproduce |
|---|---|---|
| The centerpoint | a unique, determinable origin from which all points are defined by light-speed measurement in absolute time | L.O.R.E.: C₀ = H(q₀, 0) is determined, not chosen — every point in the engine is defined from C₀ by geodesic distance |
| 6-choose-4 = 15 axes | discrete 4-D lattice scaffolding, 5 triplets, 5 light-cone pairs | C(6,4) = **15** exactly; the count is internally consistent |
| High-density lattice-circle radii | discrete mass scales from "available" radii (electron 85/36, quark 325/60, proton 1105/108) | ratios reproduce: 1105/85 = **13.0**; 36:60:108 = **3:5:9**; 85 = 5·17, 325 = 5²·13, 1105 = 5·13·17 |
| Quantum loops, node-to-node energy transfer over crossable distances | local, distance-capped transfer; three loops per particle; ~65% (major) vs ~90% (minor) confinement overlap | k-NN consensus flow (T67/T72): each neuron moves only toward its k nearest neighbors, spacing-capped |
| Gravity as loop disturbance | background field generated by local structure, no separate graviton | crease density: the background fold structure (Kawasaki + soft crease diagnostics) |
| The photon as excess resonant energy released from loop nodes | emission/absorption as energy transfer off a quantized loop | anomaly/quarantine: excess repulsion energy pushed to the boundary r → 0.95 |

### 11.3 Engine Correspondence

The engine's origin node at $(0,0)$ is the centerpoint: C₀ radiates outward by
tier exactly as the patent's centerpoint radiates waves outward at light speed.
The 15-axis combinatorial count is a *coincidence of the number 15* with the
15 odd k-families (k < 30) of the googol census — noted, not asserted.

The strongest honest correspondence is the **mass-scale regularity**:
McGrath's claim that discrete scales fall out of "available" lattice-circle
sets is the same genre as C7's claim that Mersenne-gap primes map to closed
geodesics. Both assert that a discrete spectrum is forced by a geometry's
available lattice. The patent does this by assertion; the repo does it by
measured bridge values (186 primes < 10¹⁰⁰). That is the entire difference —
and the reason the repo's version is citable and the patent's is not.

### 11.4 Honest Limits

- The patent's claim-2 geometry (15 axes separated by equal angles of
  arcsin(⅓) ≈ 19.47° in R⁶) is **not reproducible**: the 15 coordinate
  4-planes are pairwise at 90°, and 15 equiangular lines at arcsin(⅓)
  saturate the relative bound (≤ 16) without a demonstrated existence
  construction. Treated as illustrative, not geometry.
- Its gravity/EM ratio (9.39e-39 for Iron) agrees with α_G = (m_p/m_Pl)² ≈
  5.91e-39 only to within 1.6× — order of magnitude, nothing more.
- Nothing in this mapping makes the patent's claims, or the PUM's own
  narrative claims, verified. The audit's §3 ruling stands: the PUM's
  cosmology is not citable as verified beyond the specific engine claims
  that survive testing.
...
- McGrath, T. S. (2007). Physical quantum model for the atom. U.S. Patent 7,284,987 B2 (Elemetric LLC; priority 2002-10-11; expired 2024-04-24). See `docs/US7284987B2_ANALYSIS.md`.
