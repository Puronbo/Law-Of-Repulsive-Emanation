...
| **MILLENNIUM PROOF: Reduction to dynamic GN inequality** | docs/MILLENNIUM_PROOF.md + audit_millennium_proof.py | Three-step proof: (1) Dynamic GN bounded via bouncing (AUDIT: Case A has gap — F bounded when Z->0 not rigorously proved, but verified numerically + asymptotically), (2) Prodi-Serrin u in L^2(L^inf) from energy dissipation (correct), (3) Serrin theorem => smooth for all t (correct). | **CONCRETE (reduction, not complete proof)** |
