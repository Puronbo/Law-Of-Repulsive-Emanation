...
All four methods are used in our 79 experiments. The numerical method (3) is the most universal — it works for any computable f and g. The algebraic method (4) is the most powerful — it can give exact results.

### 2.2 The removable value is the theorem

In each of the 79 experiments, the removable value *is* the quantity that the theorem asserts:
...
In our 79 experiments, we compute the removable values numerically. The results are consistent with the theorems. But no finite computation can *prove* a theorem about all zeros, all functions, or all manifolds.
...
*Computational data from the repository Puronbo/Law-Of-Repulsive-Emanation. All 79 experiments verified. 669 regression tests passing. The honest wall stands.*
