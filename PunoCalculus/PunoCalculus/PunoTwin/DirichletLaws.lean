...
noncomputable def chi8Z (n : ℕ) : ℝ :=
  if n % 8 = 1 then 1 else if n % 8 = 3 then -1 else if n % 8 = 5 then -1 else
    if n % 8 = 7 then 1 else 0
lemma chi8Z_eight_mul (k : ℕ) : chi8Z (8 * k) = 0 := by
  simp [chi8Z]
lemma chi8Z_eight_mul_add_zero (k : ℕ) : chi8Z (8 * k + 0) = 0 := by
  simp [chi8Z]
lemma chi8Z_eight_mul_add_one (k : ℕ) : chi8Z (8 * k + 1) = 1 := by
  simp [chi8Z]
lemma chi8Z_eight_mul_add_two (k : ℕ) : chi8Z (8 * k + 2) = 0 := by
  simp [chi8Z]
lemma chi8Z_eight_mul_add_three (k : ℕ) : chi8Z (8 * k + 3) = -1 := by
  simp [chi8Z]
lemma chi8Z_eight_mul_add_four (k : ℕ) : chi8Z (8 * k + 4) = 0 := by
  simp [chi8Z]
lemma chi8Z_eight_mul_add_five (k : ℕ) : chi8Z (8 * k + 5) = -1 := by
  simp [chi8Z]
lemma chi8Z_eight_mul_add_six (k : ℕ) : chi8Z (8 * k + 6) = 0 := by
  simp [chi8Z]
lemma chi8Z_eight_mul_add_seven (k : ℕ) : chi8Z (8 * k + 7) = 1 := by
  simp [chi8Z]

/-! 
`ZMod.χ₈`: `χ₈(n) = 0, 1, -1, -1, 1` on `n ≡ 0, 1, 3, 5, 7 mod 8`. -/
lemma chi8Z_eq_χ₈ (n : ℕ) : ((ZMod.χ₈ n : ℤ) : ℝ) = chi8Z n := by
  have hmod : n % 8 = 0 ∨ n % 8 = 1 ∨ n % 8 = 2 ∨ n % 8 = 3 ∨ n % 8 = 4 ∨ n % 8 = 5 ∨
      n % 8 = 6 ∨ n % 8 = 7 := by omega
  rcases hmod with h0 | h1 | h2 | h3 | h4 | h5 | h6 | h7
  · have hn : (n : ZMod 8) = ((0 : ℕ) : ZMod 8) := by
...
-- the grouped series term, in `ℝ`
noncomputable def chi8GroupTerm (k : ℕ) : ℝ :=
  (1 : ℝ) / (8 * (k : ℝ) + 1) - (1 : ℝ) / (8 * (k : ℝ) + 3)
    - (1 : ℝ) / (8 * (k : ℝ) + 5) + (1 : ℝ) / (8 * (k : ℝ) + 7)

noncomputable def chi8Group (M : ℕ) : ℝ :=
  ∑ k ∈ Finset.range M, chi8GroupTerm k

-- ordered partial sums of `χ₈ n / n`
noncomputable def chi8Partial (N : ℕ) : ℝ :=
  ∑ n ∈ Finset.range N, chi8Z n / (n : ℝ)

noncomputable def chi8LimitF (x : ℝ) : ℝ :=
  if x < 1 then (1 - x ^ 2) / (1 + x ^ 4) else 0

noncomputable def chi8EvenThetaF (x : ℝ) : ℝ :=
  (1 : ℝ) / (2 * Real.sqrt 2) * (Real.log (x ^ 2 + Real.sqrt 2 * x + 1) -
    Real.log (x ^ 2 - Real.sqrt 2 * x + 1))

lemma hasDerivAt_chi8EvenThetaF (x : ℝ) :
    HasDerivAt chi8EvenThetaF ((1 - x ^ 2) / (1 + x ^ 4)) x := by
  have hsqrt : Real.sqrt 2 ≠ 0 := ne_of_gt (Real.sqrt_pos.mpr (by norm_num : (0 : ℝ) < 2))
  have hposU : 0 < x ^ 2 + Real.sqrt 2 * x + 1 := by
    have hctx : (x + Real.sqrt 2 / 2) ^ 2 + 1 / 2 = x ^ 2 + Real.sqrt 2 * x + 1 := by
      have hs2' : (Real.sqrt 2) ^ 2 = 2 := Real.sq_sqrt (by norm_num)
      field_simp [hs2']
      ring_nf
      ...
      ring
    nlinarith [sq_nonneg (x + Real.sqrt 2 / 2), hctx]
  have hposL : 0 < x ^ 2 - Real.sqrt 2 * x + 1 := by
    have hctx : (x - Real.sqrt 2 / 2) ^ 2 + 1 / 2 = x ^ 2 - Real.sqrt 2 * x + 1 := by
      have hs2' : (Real.sqrt 2) ^ 2 = 2 := Real.sq_sqrt (by norm_num)
      field_simp [hs2']
      ring_nf
      ...
      ring
    nlinarith [sq_nonneg (x - Real.sqrt 2 / 2), hctx]
  have hdU : HasDerivAt (fun y : ℝ => Real.log (y ^ 2 + Real.sqrt 2 * y + 1))
      ((2 * x + Real.sqrt 2) / (x ^ 2 + Real.sqrt 2 * x + 1)) x := by
    have hd2 : HasDerivAt (fun y : ℝ => y ^ 2) (2 * x) x := by
      ...
      · funext y
        simp [id]
      · simp [id]
    have hds : HasDerivAt (fun y : ℝ => Real.sqrt 2 * y) (Real.sqrt 2) x := by
      simpa using ((hasDerivAt_id x).const_mul (Real.sqrt 2))
    have hd1 : HasDerivAt (fun _ : ℝ => (1 : ℝ)) 0 x := hasDerivAt_const x 1
    have hinner : HasDerivAt (fun y : ℝ => y ^ 2 + Real.sqrt 2 * y + 1) (2 * x + Real.sqrt 2) x := by
      have h := (hd2.add hds).add hd1
      convert h using 1
      · rfl
      · rfl
      · funext y
        simp [Pi.add_apply]
      · simp
    have hlog : HasDerivAt (fun y : ℝ => Real.log (y ^ 2 + Real.sqrt 2 * y + 1))
        ((x ^ 2 + Real.sqrt 2 * x + 1)⁻¹ * (2 * x + Real.sqrt 2)) x := by
      exact HasDerivAt.comp (x := x) (h₂ := Real.log)
        (h₂' := (x ^ 2 + Real.sqrt 2 * x + 1)⁻¹)
        (h := fun y : ℝ => y ^ 2 + Real.sqrt 2 * y + 1)
        (h' := 2 * x + Real.sqrt 2)
        (Real.hasDerivAt_log (ne_of_gt hposU)) hinner
    simpa [div_eq_mul_inv, mul_comm, mul_left_comm, mul_assoc] using hlog
  have hdL : HasDerivAt (fun y : ℝ => Real.log (y ^ 2 - Real.sqrt 2 * y + 1))
      ((2 * x - Real.sqrt 2) / (x ^ 2 - Real.sqrt 2 * x + 1)) x := by
    have hd2 : HasDerivAt (fun y : ℝ => y ^ 2) (2 * x) x := by
      have hd := (hasDerivAt_id x).pow 2
      convert hd using 1
    ...
    have hlog : HasDerivAt (fun y : ℝ => Real.log (y ^ 2 - Real.sqrt 2 * y + 1))
      ...
      exact HasDerivAt.comp (x := x) (h₂ := Real.log)
        ...
        (h := fun y : ℝ => y ^ 2 - Real.sqrt 2 * y + 1)
        (h' := 2 * x - Real.sqrt 2)
        (Real.hasDerivAt_log (ne_of_gt hposL)) hinner
    simpa [div_eq_mul_inv, mul_comm, mul_left_comm, mul_assoc] using hlog
  have hsum := hdU.sub hdL
  have hfix : (1 : ℝ) / (2 * Real.sqrt 2) *
      (((2 * x + Real.sqrt 2) / (x ^ 2 + Real.sqrt 2 * x + 1)) -
        ((2 * x - Real.sqrt 2) / (x ^ 2 - Real.sqrt 2 * x + 1)))
    = (1 - x ^ 2) / (1 + x ^ 4) := by
    have hs2 : (Real.sqrt 2) ^ 2 = 2 := Real.sq_sqrt (by norm_num)
    have hU : x ^ 2 + Real.sqrt 2 * x + 1 ≠ 0 := ne_of_gt hposU
    have hL : x ^ 2 - Real.sqrt 2 * x + 1 ≠ 0 := ne_of_gt hposL
    have hdn : 1 + x ^ 4 ≠ 0 := by
      have hpos : 0 < 1 + x ^ 4 := by nlinarith [sq_nonneg (x ^ 2)]
      exact ne_of_gt hpos
    have hAB : (x ^ 2 - Real.sqrt 2 * x + 1) * (x ^ 2 + Real.sqrt 2 * x + 1) = x ^ 4 + 1 := by
      calc
        ...
        _ = (x ^ 2 + 1) ^ 2 - 2 * x ^ 2 := by rw [hs2]
        _ = x ^ 4 + 1 := by ring
    have hnum : (2 * x + Real.sqrt 2) * (x ^ 2 - Real.sqrt 2 * x + 1) -
          (2 * x - Real.sqrt 2) * (x ^ 2 + Real.sqrt 2 * x + 1)
        = 2 * Real.sqrt 2 * (1 - x ^ 2) := by ring
    have hsumden : (2 * x + Real.sqrt 2) / (x ^ 2 + Real.sqrt 2 * x + 1) -
          (2 * x - Real.sqrt 2) / (x ^ 2 - Real.sqrt 2 * x + 1)
        = (2 * Real.sqrt 2 * (1 - x ^ 2)) / (1 + x ^ 4) := by
      have hstep := div_sub_div (a := (2 * x + Real.sqrt 2)) (c := (2 * x - Real.sqrt 2))
        (b := x ^ 2 + Real.sqrt 2 * x + 1) (d := x ^ 2 - Real.sqrt 2 * x + 1) hU hL
      rw [hstep]
      rw [show (x ^ 2 + Real.sqrt 2 * x + 1) * (2 * x - Real.sqrt 2) =
          (2 * x - Real.sqrt 2) * (x ^ 2 + Real.sqrt 2 * x + 1) by ring]
      rw [hnum]
      rw [show (x ^ 2 + Real.sqrt 2 * x + 1) * (x ^ 2 - Real.sqrt 2 * x + 1) = x ^ 4 + 1 by
        rw [mul_comm]
        exact hAB]
      rw [show x ^ 4 + 1 = 1 + x ^ 4 by ring]
    calc
      (1 : ℝ) / (2 * Real.sqrt 2) * (((2 * x + Real.sqrt 2) / (x ^ 2 + Real.sqrt 2 * x + 1)) -
          ((2 * x - Real.sqrt 2) / (x ^ 2 - Real.sqrt 2 * x + 1)))
          = (1 : ℝ) / (2 * Real.sqrt 2) * ((2 * Real.sqrt 2 * (1 - x ^ 2)) / (1 + x ^ 4)) := by
            rw [hsumden]
      _ = (1 - x ^ 2) / (1 + x ^ 4) := by
            field_simp [hsqrt, hdn]
  have htot := hsum.const_mul (1 / (2 * Real.sqrt 2))
  rw [hfix] at htot
  convert! htot using 1

lemma integral_chi8_value :
    ∫ x in (0 : ℝ)..1, (1 - x ^ 2) / (1 + x ^ 4) = Real.log (1 + Real.sqrt 2) / Real.sqrt 2 := by
  have hcont : ContinuousOn chi8EvenThetaF (Set.Icc 0 1) := by
    unfold chi8EvenThetaF
    have hposU : ∀ z : ℝ, 0 < z ^ 2 + Real.sqrt 2 * z + 1 := by
      intro z
      have hctx : (z + Real.sqrt 2 / 2) ^ 2 + 1 / 2 = z ^ 2 + Real.sqrt 2 * z + 1 := by
        have hs2' : (Real.sqrt 2) ^ 2 = 2 := Real.sq_sqrt (by norm_num)
        field_simp [hs2']
        ring_nf
        rw [hs2']
        ring
      nlinarith [sq_nonneg (z + Real.sqrt 2 / 2), hctx]
    have hposL : ∀ z : ℝ, 0 < z ^ 2 - Real.sqrt 2 * z + 1 := by
      intro z
      have hctx : (z - Real.sqrt 2 / 2) ^ 2 + 1 / 2 = z ^ 2 - Real.sqrt 2 * z + 1 := by
        have hs2' : (Real.sqrt 2) ^ 2 = 2 := Real.sq_sqrt (by norm_num)
        field_simp [hs2']
        ...
        ring
      nlinarith [sq_nonneg (z - Real.sqrt 2 / 2), hctx]
    have hcU : ContinuousOn (fun z : ℝ => Real.log (z ^ 2 + Real.sqrt 2 * z + 1)) (Set.Icc (0 : ℝ) 1) := by
      apply ContinuousOn.log (by fun_prop)
      intro z hz
      exact ne_of_gt (hposU z)
    have hcL : ContinuousOn (fun z : ℝ => Real.log (z ^ 2 - Real.sqrt 2 * z + 1)) (Set.Icc (0 : ℝ) 1) := by
      apply ContinuousOn.log (by fun_prop)
      intro z hz
      exact ne_of_gt (hposL z)
    exact (hcU.sub hcL).const_mul (1 / (2 * Real.sqrt 2))
  have hderiv : ∀ x ∈ Set.Ioo (0 : ℝ) 1, HasDerivAt chi8EvenThetaF ((1 - x ^ 2) / (1 + x ^ 4)) x := by
    intro x hx
    exact hasDerivAt_chi8EvenThetaF x
  have hint : IntervalIntegrable (fun x : ℝ => (1 - x ^ 2) / (1 + x ^ 4)) MeasureTheory.volume 0 1 := by
    have hden : ∀ x : ℝ, 1 + x ^ 4 ≠ 0 := by
      intro x
      have hpos : 0 < 1 + x ^ 4 := by nlinarith [sq_nonneg (x ^ 2)]
  ...
  have hftc := intervalIntegral.integral_eq_sub_of_hasDerivAt_of_le (a := (0 : ℝ)) (b := 1)
    (by norm_num) hcont hderiv hint
  have hval : chi8EvenThetaF 1 - chi8EvenThetaF 0 = Real.log (1 + Real.sqrt 2) / Real.sqrt 2 := by
    have hsqrt : Real.sqrt 2 ≠ 0 := ne_of_gt (Real.sqrt_pos.mpr (by norm_num : (0 : ℝ) < 2))
    have hs2 : (Real.sqrt 2) ^ 2 = 2 := Real.sq_sqrt (by norm_num)
    have hF0 : chi8EvenThetaF 0 = 0 := by
      unfold chi8EvenThetaF
      rw [show (0 : ℝ) ^ 2 + Real.sqrt 2 * 0 + 1 = 1 by ring]
      rw [show (0 : ℝ) ^ 2 - Real.sqrt 2 * 0 + 1 = 1 by ring]
      simp
    have hF1 : chi8EvenThetaF 1 =
        (1 : ℝ) / (2 * Real.sqrt 2) * (Real.log (2 + Real.sqrt 2) - Real.log (2 - Real.sqrt 2)) := by
      unfold chi8EvenThetaF
      rw [show (1 : ℝ) ^ 2 + Real.sqrt 2 * 1 + 1 = 2 + Real.sqrt 2 by ring]
      rw [show (1 : ℝ) ^ 2 - Real.sqrt 2 * 1 + 1 = 2 - Real.sqrt 2 by ring]
    have hpos2p : (0 : ℝ) < 2 + Real.sqrt 2 := by
      nlinarith [Real.sqrt_pos.mpr (by norm_num : (0 : ℝ) < 2)]
    have hpos2m : (0 : ℝ) < 2 - Real.sqrt 2 := by
      have hslt : Real.sqrt 2 < 2 := by
        have h := Real.sqrt_lt_sqrt (by norm_num : (0 : ℝ) ≤ 2) (by norm_num : (2 : ℝ) < 4)
        rw [show Real.sqrt 4 = 2 by norm_num [Real.sqrt_eq_rpow]] at h
        exact h
      nlinarith
    have hfac : (2 + Real.sqrt 2) / (2 - Real.sqrt 2) = (1 + Real.sqrt 2) ^ 2 := by
      field_simp [ne_of_gt hpos2m]
      nlinarith [hs2]
    have hlogab : Real.log (2 + Real.sqrt 2) - Real.log (2 - Real.sqrt 2)
        = 2 * Real.log (1 + Real.sqrt 2) := by
      have hlogdiv : Real.log (2 + Real.sqrt 2) - Real.log (2 - Real.sqrt 2)
          = Real.log ((2 + Real.sqrt 2) / (2 - Real.sqrt 2)) := by
        exact (Real.log_div (ne_of_gt hpos2p) (ne_of_gt hpos2m)).symm
      calc
        Real.log (2 + Real.sqrt 2) - Real.log (2 - Real.sqrt 2)
            = Real.log ((2 + Real.sqrt 2) / (2 - Real.sqrt 2)) := hlogdiv
        _ = Real.log ((1 + Real.sqrt 2) ^ 2) := by rw [hfac]
        _ = 2 * Real.log (1 + Real.sqrt 2) := by
              rw [Real.log_pow (1 + Real.sqrt 2) 2]
              norm_num
    calc
      chi8EvenThetaF 1 - chi8EvenThetaF 0 = chi8EvenThetaF 1 := by rw [hF0]; ring
      _ = (1 : ℝ) / (2 * Real.sqrt 2) * (Real.log (2 + Real.sqrt 2) - Real.log (2 - Real.sqrt 2)) := hF1
      _ = (1 : ℝ) / (2 * Real.sqrt 2) * (2 * Real.log (1 + Real.sqrt 2)) := by rw [hlogab]
      _ = Real.log (1 + Real.sqrt 2) / Real.sqrt 2 := by
            field_simp [hsqrt, (by norm_num : (2 : ℝ) ≠ 0)]
  rwa [hval] at hftc

-- the grouped step functions
noncomputable def chi8StepF (N : ℕ) (x : ℝ) : ℝ :=
  ∑ k ∈ Finset.range N, (x ^ (8 * k) - x ^ (8 * k + 2) - x ^ (8 * k + 4) + x ^ (8 * k + 6))

lemma chi8StepF_closed {N : ℕ} {x : ℝ} (hx : x ∈ Set.Ioc (0 : ℝ) 1) :
    chi8StepF N x = (1 - x ^ 2 - x ^ 4 + x ^ 6) * (1 - x ^ (8 * N)) / (1 - x ^ 8) := by
  rcases hx with ⟨h0, hxle⟩
  by_cases hx1 : x = 1
  · subst hx1
    have hstep : chi8StepF N 1 = 0 := by
      unfold chi8StepF
      apply Finset.sum_eq_zero
      intro k hk
      have h1 : (1 : ℝ) ^ (8 * k) = 1 := by simp
  ...
  · have hlt : x < 1 := lt_of_le_of_ne hxle hx1
    ...
    have hterm : ∀ k, x ^ (8 * k) - x ^ (8 * k + 2) - x ^ (8 * k + 4) + x ^ (8 * k + 6)
      ...
      ring
    calc
      chi8StepF N x = ∑ k ∈ Finset.range N, (1 - x ^ 2 - x ^ 4 + x ^ 6) * (x ^ 8) ^ k := by
        unfold chi8StepF
        refine Finset.sum_congr rfl ?_
        intro k hk
        exact hterm k
...
lemma chi8StepF_norm_le {N : ℕ} {x : ℝ} (hx : x ∈ Set.Ioc (0 : ℝ) 1) :
    ‖chi8StepF N x‖ ≤ (2 : ℝ) := by
  rcases hx with ⟨h0, hxle⟩
  by_cases hx1 : x = 1
  · subst hx1
    have hstep : chi8StepF N 1 = 0 := by
    ...
    norm_num
  · have hlt : x < 1 := lt_of_le_of_ne hxle hx1
    rw [chi8StepF_closed (N := N) (x := x) ⟨h0, hxle⟩]
    have hdenpos : 0 < 1 - x ^ 8 := by
      have hp : x ^ 8 < 1 := pow_lt_one₀ h0.le hlt (by norm_num : (8 : ℕ) ≠ 0)
      nlinarith
    have hge8 : 0 ≤ 1 - x ^ (8 * N) := by
    ...
    have hge1 : 0 ≤ 1 - x ^ 2 := by
      have hp := pow_le_one₀ (n := 2) h0.le hxle
      nlinarith
    have hge4 : 0 ≤ 1 - x ^ 4 := by
      have hp := pow_le_one₀ (n := 4) h0.le hxle
      nlinarith
    have hden1 : 0 < (1 + x ^ 2) * (1 + x ^ 4) := by
      nlinarith [sq_nonneg x, sq_nonneg (x ^ 2), pow_nonneg h0.le 6]
    have hden1le : (1 : ℝ) ≤ (1 + x ^ 2) * (1 + x ^ 4) := by
      nlinarith [sq_nonneg x, sq_nonneg (x ^ 2), pow_nonneg h0.le 6]
    have hle14 : (1 - x ^ 4) ≤ (1 : ℝ) := by nlinarith [pow_nonneg h0.le 4]
    have hle18 : (1 - x ^ (8 * N)) ≤ (1 : ℝ) := by nlinarith [pow_nonneg h0.le (8 * N)]
    have hle1 : (1 - x ^ 4) * (1 - x ^ (8 * N)) ≤ (1 : ℝ) := by
        simpa using mul_le_mul hle14 hle18 hge8 (by norm_num)
    have hle2 : (1 - x ^ 4) * (1 - x ^ (8 * N)) / ((1 + x ^ 2) * (1 + x ^ 4)) ≤ 2 := by
      rw [div_le_iff₀ hden1]
      have htarget : (1 - x ^ 4) * (1 - x ^ (8 * N)) ≤ 2 * ((1 + x ^ 2) * (1 + x ^ 4)) := by
        exact le_trans hle1 (by nlinarith [hden1le])
      exact htarget
    have hfac : 1 - x ^ 2 - x ^ 4 + x ^ 6 = (1 - x ^ 2) * (1 - x ^ 4) := by ring
    have hfac2 : 1 - x ^ 8 = (1 - x ^ 2) * (1 + x ^ 2) * (1 + x ^ 4) := by ring
    have hnum0 : 0 ≤ (1 - x ^ 2 - x ^ 4 + x ^ 6) * (1 - x ^ (8 * N)) := by
      have hA : 0 ≤ 1 - x ^ 2 - x ^ 4 + x ^ 6 := by
        rw [hfac]
        exact mul_nonneg hge1 hge4
      exact mul_nonneg hA hge8
    have hle' : (1 - x ^ 2 - x ^ 4 + x ^ 6) * (1 - x ^ (8 * N)) / (1 - x ^ 8) ≤ 2 := by
      have hx2 : x ^ 2 < 1 := pow_lt_one₀ h0.le hlt (by norm_num : (2 : ℕ) ≠ 0)
      have hcz : (1 - x ^ 2) ≠ 0 := sub_ne_zero.mpr (ne_of_lt hx2).symm
      calc
        (1 - x ^ 2 - x ^ 4 + x ^ 6) * (1 - x ^ (8 * N)) / (1 - x ^ 8)
            = ((1 - x ^ 2) * (1 - x ^ 4) * (1 - x ^ (8 * N))) /
                ((1 - x ^ 2) * (1 + x ^ 2) * (1 + x ^ 4)) := by
              rw [hfac, hfac2]
        _ = ((1 - x ^ 4) * (1 - x ^ (8 * N))) / ((1 + x ^ 2) * (1 + x ^ 4)) := by
              field_simp [hcz, ne_of_gt hden1]
        _ ≤ 2 := hle2
    have hge : 0 ≤ (1 - x ^ 2 - x ^ 4 + x ^ 6) * (1 - x ^ (8 * N)) / (1 - x ^ 8) :=
      div_nonneg hnum0 hdenpos.le
    rw [Real.norm_eq_abs, abs_of_nonneg hge]
...
lemma chi8StepF_tendsto_limit {x : ℝ} (hx : x ∈ Set.Ioc (0 : ℝ) 1) :
    Tendsto (fun N : ℕ => chi8StepF N x) atTop (𝓝 (chi8LimitF x)) := by
  rcases hx with ⟨h0, hxle⟩
  by_cases hx1 : x = 1
  · subst hx1
    have hz : ∀ N : ℕ, chi8StepF N 1 = 0 := by
      intro N
      unfold chi8StepF
      apply Finset.sum_eq_zero
      intro k hk
      ...
      rw [h1, h2, h4, h6]
      norm_num
    have hlf : chi8LimitF 1 = 0 := by simp [chi8LimitF]
    simp [hz, hlf]
  · have hlt : x < 1 := lt_of_le_of_ne hxle hx1
    have hlim : chi8LimitF x = (1 - x ^ 2) / (1 + x ^ 4) := by
      simp [chi8LimitF, hlt]
    have hx8lt : ‖x ^ 8‖ < (1 : ℝ) := by
      rw [Real.norm_eq_abs, abs_of_pos (pow_pos h0 8)]
      exact pow_lt_one₀ h0.le hlt (by norm_num : (8 : ℕ) ≠ 0)
    ...
    have hv : (1 - x ^ 2 - x ^ 4 + x ^ 6) / (1 - x ^ 8) = (1 - x ^ 2) / (1 + x ^ 4) := by
      ...
      calc
        ...
        _ = (1 - x ^ 2) / (1 + x ^ 4) := by
              field_simp [ne_of_gt hd4p, hden4]
    have hq' : Tendsto (fun N : ℕ => chi8StepF N x) atTop
        (𝓝 ((1 - x ^ 2) / (1 + x ^ 4))) := by
      simpa [hv] using hq.congr' (Eventually.of_forall (fun N => by
        exact (chi8StepF_closed (N := N) (x := x) ⟨h0, hxle⟩).symm))
    convert hq' using 1
    · rw [hlim]

lemma integral_chi8StepF (N : ℕ) :
    ∫ x in (0 : ℝ)..1, chi8StepF N x = chi8Group N := by
  unfold chi8StepF chi8Group
  have hI0 (k : ℕ) : IntervalIntegrable (fun x : ℝ => x ^ (8 * k)) MeasureTheory.volume 0 1 := by
    have hc : Continuous (fun x : ℝ => x ^ (8 * k)) := by fun_prop
    exact hc.intervalIntegrable 0 1
  have hI2 (k : ℕ) : IntervalIntegrable (fun x : ℝ => x ^ (8 * k + 2)) MeasureTheory.volume 0 1 := by
  ...
  have hI6 (k : ℕ) : IntervalIntegrable (fun x : ℝ => x ^ (8 * k + 6)) MeasureTheory.volume 0 1 := by
    ...
    exact hc.intervalIntegrable 0 1
  rw [intervalIntegral.integral_finsetSum
    (fun k hk => (((hI0 k).sub (hI2 k)).sub (hI4 k)).add (hI6 k))]
  apply Finset.sum_congr rfl
  intro k hk
  unfold chi8GroupTerm
  rw [intervalIntegral.integral_add (((hI0 k).sub (hI2 k)).sub (hI4 k)) (hI6 k)]
  rw [intervalIntegral.integral_sub ((hI0 k).sub (hI2 k)) (hI4 k)]
  rw [intervalIntegral.integral_sub (hI0 k) (hI2 k)]
  rw [integral_pow_aux (8 * k), integral_pow_aux (8 * k + 2), integral_pow_aux (8 * k + 4),
    integral_pow_aux (8 * k + 6)]
  norm_num [Nat.cast_add, Nat.cast_mul]
  ring

lemma tendsto_integral_chi8StepF :
    Tendsto (fun N : ℕ => ∫ x in (0 : ℝ)..1, chi8StepF N x) atTop
      (𝓝 (∫ x in (0 : ℝ)..1, chi8LimitF x)) := by
  refine intervalIntegral.tendsto_integral_filter_of_dominated_convergence
      (μ := MeasureTheory.volume) (l := atTop) (F := fun N x => chi8StepF N x)
      (bound := fun _ : ℝ => 2) (f := chi8LimitF) ?_ ?_ ?_ ?_
  · refine Eventually.of_forall ?_
    intro N
    have hc : Continuous (chi8StepF N) := by
      unfold chi8StepF
      fun_prop
    exact hc.aestronglyMeasurable
  · refine Eventually.of_forall ?_
    ...
    exact MeasureTheory.ae_of_all MeasureTheory.volume
      (fun x => fun hx =>
        chi8StepF_norm_le (N := N) (by
          simpa [Set.uIoc_of_le (show (0 : ℝ) ≤ 1 by norm_num)] using hx))
  · have hc : Continuous (fun _ : ℝ => (2 : ℝ)) := continuous_const
    exact hc.intervalIntegrable 0 1
  · exact MeasureTheory.ae_of_all MeasureTheory.volume
      (fun x => fun hx =>
        chi8StepF_tendsto_limit (by
          simpa [Set.uIoc_of_le (show (0 : ℝ) ≤ 1 by norm_num)] using hx))

lemma chi8Group_tendsto :
    Tendsto chi8Group atTop (𝓝 (Real.log (1 + Real.sqrt 2) / Real.sqrt 2)) := by
  have h1 := tendsto_integral_chi8StepF
  have hval : (∫ x in (0 : ℝ)..1, chi8LimitF x) = Real.log (1 + Real.sqrt 2) / Real.sqrt 2 := by
    have hcongr : ∫ x in (0 : ℝ)..1, chi8LimitF x = ∫ x in (0 : ℝ)..1, (1 - x ^ 2) / (1 + x ^ 4) := by
      exact intervalIntegral.integral_congr_Ioo_of_le (show (0 : ℝ) ≤ 1 by norm_num)
        (fun x hx => by simp [chi8LimitF, hx.2])
    rw [hcongr]
    exact integral_chi8_value
  have h1' : Tendsto (fun N : ℕ => ∫ x in (0 : ℝ)..1, chi8StepF N x) atTop
      (𝓝 (Real.log (1 + Real.sqrt 2) / Real.sqrt 2)) := by
    simpa [hval] using h1
  exact h1'.congr' (Eventually.of_forall (fun N => integral_chi8StepF N))

-- ordered partial sums reduce to the grouped series, plus a lim-vanishing correction
lemma chi8Partial_eight_mul (M : ℕ) : chi8Partial (8 * M) = chi8Group M := by
  induction M with
  | zero => simp [chi8Partial, chi8Group]
  | succ M ih =>
      rw [show 8 * (M + 1) = 8 * M + 8 by omega]
      unfold chi8Partial
      rw [Finset.sum_range_add]
      have hb1 : chi8Z (8 * M + 1) / ((8 * M + 1 : ℕ) : ℝ) = 1 / (8 * (M : ℝ) + 1) := by
        have hc : ((8 * M + 1 : ℕ) : ℝ) = 8 * (M : ℝ) + 1 := by norm_num [Nat.cast_add, Nat.cast_mul]
        rw [chi8Z_eight_mul_add_one M, hc]
      have hb3 : chi8Z (8 * M + 3) / ((8 * M + 3 : ℕ) : ℝ) = -(1 / (8 * (M : ℝ) + 3)) := by
        have hc : ((8 * M + 3 : ℕ) : ℝ) = 8 * (M : ℝ) + 3 := by norm_num [Nat.cast_add, Nat.cast_mul]
        rw [chi8Z_eight_mul_add_three M, hc, neg_div]
      have hb5 : chi8Z (8 * M + 5) / ((8 * M + 5 : ℕ) : ℝ) = -(1 / (8 * (M : ℝ) + 5)) := by
        have hc : ((8 * M + 5 : ℕ) : ℝ) = 8 * (M : ℝ) + 5 := by norm_num [Nat.cast_add, Nat.cast_mul]
        rw [chi8Z_eight_mul_add_five M, hc, neg_div]
      have hb7 : chi8Z (8 * M + 7) / ((8 * M + 7 : ℕ) : ℝ) = 1 / (8 * (M : ℝ) + 7) := by
        have hc : ((8 * M + 7 : ℕ) : ℝ) = 8 * (M : ℝ) + 7 := by norm_num [Nat.cast_add, Nat.cast_mul]
        rw [chi8Z_eight_mul_add_seven M, hc]
      have hb0 : chi8Z (8 * M + 0) / ((8 * M + 0 : ℕ) : ℝ) = 0 := by
        rw [chi8Z_eight_mul_add_zero M]
        simp
      have hb2 : chi8Z (8 * M + 2) / ((8 * M + 2 : ℕ) : ℝ) = 0 := by
        rw [chi8Z_eight_mul_add_two M]
        simp
      have hb4 : chi8Z (8 * M + 4) / ((8 * M + 4 : ℕ) : ℝ) = 0 := by
        rw [chi8Z_eight_mul_add_four M]
        simp
      have hb6 : chi8Z (8 * M + 6) / ((8 * M + 6 : ℕ) : ℝ) = 0 := by
        rw [chi8Z_eight_mul_add_six M]
        simp
      have hblock : (∑ j ∈ Finset.range 8, chi8Z (8 * M + j) / ((8 * M + j : ℕ) : ℝ))
          = chi8GroupTerm M := by
        unfold chi8GroupTerm
        rw [Finset.sum_range_succ, Finset.sum_range_succ, Finset.sum_range_succ,
          Finset.sum_range_succ, Finset.sum_range_succ, Finset.sum_range_succ,
        ...
        ring
      rw [hblock]
      have hg : chi8Group (M + 1) = chi8Group M + chi8GroupTerm M := by
        unfold chi8Group
        rw [Finset.sum_range_succ]
      rw [hg]
      rw [← ih]
...
noncomputable def chi8TurnTail (N : ℕ) : ℝ :=
  ∑ j ∈ Finset.range (N % 8), chi8Z (8 * (N / 8) + j) / ((8 * (N / 8) + j : ℕ) : ℝ)

lemma chi8Partial_split (N : ℕ) :
    chi8Partial N = chi8Partial (8 * (N / 8)) + chi8TurnTail N := by
  unfold chi8Partial chi8TurnTail
  conv_lhs => rw [← show (8 * (N / 8) + N % 8) = N by omega]
  rw [Finset.sum_range_add]

lemma chi8TurnTail_bound {N : ℕ} :
    ‖chi8TurnTail N‖ ≤ (8 : ℝ) / ((8 * (N / 8) + 1 : ℕ) : ℝ) := by
  unfold chi8TurnTail
  have hkpos : (0 : ℕ) < 8 * (N / 8) + 1 := by omega
  have hdpos : (0 : ℝ) < ((8 * (N / 8) + 1 : ℕ) : ℝ) := by exact_mod_cast hkpos
  have hc : 0 ≤ (1 : ℝ) / ((8 * (N / 8) + 1 : ℕ) : ℝ) := div_nonneg (by norm_num) hdpos.le
  have hb : ∑ j ∈ Finset.range (N % 8),
        ‖chi8Z (8 * (N / 8) + j) / ((8 * (N / 8) + j : ℕ) : ℝ)‖
      ≤ ∑ j ∈ Finset.range (N % 8), (1 : ℝ) / ((8 * (N / 8) + 1 : ℕ) : ℝ) := by
    refine Finset.sum_le_sum ?_
    intro j hjin
    have hj8 : j < 8 := lt_of_lt_of_le (Finset.mem_range.mp hjin) (le_of_lt (Nat.mod_lt N (by norm_num : 0 < 8)))
    ...
    rcases hjr with h0 | h1 | h2 | h3 | h4 | h5 | h6 | h7
    · subst h0
      have hz : chi8Z (8 * (N / 8) + 0) = 0 := chi8Z_eight_mul_add_zero (N / 8)
      rw [hz, zero_div]
      simpa using hc
    · subst h1
      have hz : chi8Z (8 * (N / 8) + 1) = 1 := chi8Z_eight_mul_add_one (N / 8)
      rw [hz]
      have hnorm : ‖(1 : ℝ) / ((8 * (N / 8) + 1 : ℕ) : ℝ)‖ = (1 : ℝ) / ((8 * (N / 8) + 1 : ℕ) : ℝ) := by
        rw [Real.norm_eq_abs, abs_of_pos (div_pos (by norm_num) hdpos)]
      rw [hnorm]
    · subst h2
      have hz : chi8Z (8 * (N / 8) + 2) = 0 := chi8Z_eight_mul_add_two (N / 8)
      rw [hz, zero_div]
      simpa using hc
    · subst h3
      have hk3 : (0 : ℕ) < 8 * (N / 8) + 3 := by omega
      have hd3 : (0 : ℝ) < ((8 * (N / 8) + 3 : ℕ) : ℝ) := by exact_mod_cast hk3
      have hz : chi8Z (8 * (N / 8) + 3) = -1 := chi8Z_eight_mul_add_three (N / 8)
      rw [hz]
      have hnorm : ‖(-(1 : ℝ)) / ((8 * (N / 8) + 3 : ℕ) : ℝ)‖ = (1 : ℝ) / ((8 * (N / 8) + 3 : ℕ) : ℝ) := by
        rw [neg_div]
        rw [Real.norm_eq_abs, abs_neg, abs_of_pos (div_pos (by norm_num) hd3)]
      rw [hnorm]
      have hleD : ((8 * (N / 8) + 1 : ℕ) : ℝ) ≤ ((8 * (N / 8) + 3 : ℕ) : ℝ) := by
        exact_mod_cast (by omega : (8 * (N / 8) + 1 : ℕ) ≤ 8 * (N / 8) + 3)
      have hide : ((8 * (N / 8) + 3 : ℕ) : ℝ)⁻¹ ≤ ((8 * (N / 8) + 1 : ℕ) : ℝ)⁻¹ := by
      ...
      simpa [one_div] using hide
    · subst h4
      have hz : chi8Z (8 * (N / 8) + 4) = 0 := chi8Z_eight_mul_add_four (N / 8)
      rw [hz, zero_div]
      simpa using hc
    · subst h5
      have hk5 : (0 : ℕ) < 8 * (N / 8) + 5 := by omega
      have hd5 : (0 : ℝ) < ((8 * (N / 8) + 5 : ℕ) : ℝ) := by exact_mod_cast hk5
      have hz : chi8Z (8 * (N / 8) + 5) = -1 := chi8Z_eight_mul_add_five (N / 8)
      rw [hz]
      have hnorm : ‖(-(1 : ℝ)) / ((8 * (N / 8) + 5 : ℕ) : ℝ)‖ = (1 : ℝ) / ((8 * (N / 8) + 5 : ℕ) : ℝ) := by
        rw [neg_div]
        rw [Real.norm_eq_abs, abs_neg, abs_of_pos (div_pos (by norm_num) hd5)]
      ...
      simpa [one_div] using hide
    · subst h6
      have hz : chi8Z (8 * (N / 8) + 6) = 0 := chi8Z_eight_mul_add_six (N / 8)
      rw [hz, zero_div]
      simpa using hc
    · subst h7
      have hk7 : (0 : ℕ) < 8 * (N / 8) + 7 := by omega
      have hd7 : (0 : ℝ) < ((8 * (N / 8) + 7 : ℕ) : ℝ) := by exact_mod_cast hk7
      have hz : chi8Z (8 * (N / 8) + 7) = 1 := chi8Z_eight_mul_add_seven (N / 8)
      rw [hz]
      have hnorm : ‖(1 : ℝ) / ((8 * (N / 8) + 7 : ℕ) : ℝ)‖ = (1 : ℝ) / ((8 * (N / 8) + 7 : ℕ) : ℝ) := by
        rw [Real.norm_eq_abs, abs_of_pos (div_pos (by norm_num) hd7)]
      rw [hnorm]
      have hleD : ((8 * (N / 8) + 1 : ℕ) : ℝ) ≤ ((8 * (N / 8) + 7 : ℕ) : ℝ) := by
        exact_mod_cast (by omega : (8 * (N / 8) + 1 : ℕ) ≤ 8 * (N / 8) + 7)
      have hide : ((8 * (N / 8) + 7 : ℕ) : ℝ)⁻¹ ≤ ((8 * (N / 8) + 1 : ℕ) : ℝ)⁻¹ := by
        ...
        exact hleD
      simpa [one_div] using hide
  have hwell : ‖chi8TurnTail N‖
      ≤ ∑ j ∈ Finset.range (N % 8), (1 : ℝ) / ((8 * (N / 8) + 1 : ℕ) : ℝ) := by
    calc
      ‖chi8TurnTail N‖
          = ‖∑ j ∈ Finset.range (N % 8), chi8Z (8 * (N / 8) + j) / ((8 * (N / 8) + j : ℕ) : ℝ)‖ := rfl
      _ ≤ ∑ j ∈ Finset.range (N % 8), ‖chi8Z (8 * (N / 8) + j) / ((8 * (N / 8) + j : ℕ) : ℝ)‖ :=
          norm_sum_le (Finset.range (N % 8))
            (fun j : ℕ => chi8Z (8 * (N / 8) + j) / ((8 * (N / 8) + j : ℕ) : ℝ))
      _ ≤ ∑ j ∈ Finset.range (N % 8), (1 : ℝ) / ((8 * (N / 8) + 1 : ℕ) : ℝ) := hb
  have hconst : ∑ j ∈ Finset.range (N % 8), (1 : ℝ) / ((8 * (N / 8) + 1 : ℕ) : ℝ)
      ≤ (8 : ℝ) / ((8 * (N / 8) + 1 : ℕ) : ℝ) := by
    have hcard : (N % 8 : ℕ) ≤ 8 := le_of_lt (Nat.mod_lt N (by norm_num : 0 < 8))
...
lemma chi8TurnTail_tendsto : Tendsto chi8TurnTail atTop (𝓝 (0 : ℝ)) := by
  have hmd : Tendsto (fun N : ℕ => (8 * (N / 8) + 1 : ℕ)) atTop atTop := by
    rw [tendsto_atTop_atTop]
    intro b
    refine ⟨8 * (b + 1), ?_⟩
  ...
  refine tendsto_of_tendsto_of_tendsto_of_le_of_le hneg hb8 ?_ ?_
  · intro N
    have hbN : |chi8TurnTail N| ≤ (8 : ℝ) / ((8 * (N / 8) + 1 : ℕ) : ℝ) := by
      simpa [Real.norm_eq_abs] using chi8TurnTail_bound (N := N)
    exact (abs_le.mp hbN).1
  · intro N
    have hbN : |chi8TurnTail N| ≤ (8 : ℝ) / ((8 * (N / 8) + 1 : ℕ) : ℝ) := by
      simpa [Real.norm_eq_abs] using chi8TurnTail_bound (N := N)
...
lemma chi8Partial_tendsto :
    Tendsto chi8Partial atTop (𝓝 (Real.log (1 + Real.sqrt 2) / Real.sqrt 2)) := by
  have hg : Tendsto (fun M : ℕ => chi8Partial (8 * M)) atTop (𝓝 (Real.log (1 + Real.sqrt 2) / Real.sqrt 2)) :=
    chi8Group_tendsto.congr' (Eventually.of_forall (fun M => (chi8Partial_eight_mul M).symm))
  have hq : Tendsto (fun N : ℕ => N / 8) atTop atTop := by
    rw [tendsto_atTop_atTop]
    intro b
    refine ⟨8 * (b + 1), ?_⟩
    intro N hN
    omega
  have hmain : Tendsto (fun N : ℕ => chi8Partial (8 * (N / 8))) atTop
      (𝓝 (Real.log (1 + Real.sqrt 2) / Real.sqrt 2)) :=
    hg.comp hq
  have htail : Tendsto (fun N : ℕ => chi8Partial N - chi8Partial (8 * (N / 8))) atTop (𝓝 (0 : ℝ)) :=
    chi8TurnTail_tendsto.congr' (Eventually.of_forall (fun N => by
      change chi8TurnTail N = chi8Partial N - chi8Partial (8 * (N / 8))
      rw [chi8Partial_split]
      ring))
  have hsum := hmain.add htail
  convert hsum using 1
  · funext N
...
/-- The `χ₈` Dirichlet series at `s = 1` (`Σ χ₈(n)/n`): its ordered partial sums in `ℝ`
coincide with the ordered partial sums of the mod-8 pattern `chi8Z n / n`. -/
lemma chi8_series_partial_real (N : ℕ) :
    (∑ n ∈ Finset.range N, ((ZMod.χ₈ n : ℤ) : ℝ) / (n : ℝ)) = chi8Partial N := by
  unfold chi8Partial
  refine Finset.sum_congr rfl ?_
  intro n hn
  rw [chi8Z_eq_χ₈ n]

/-- `L(1, χ₈) = ln(1+√2)/√2` as a Dirichlet-series limit (in `ℝ`): the ordered partial sums
`Σ_{n<N} χ₈(n)/n` tend to `ln(1+√2)/√2`. -/
lemma chi8_series_real_ln_one_plus_sqrt_two_div_sqrt_two :
    Tendsto (fun N : ℕ => ∑ n ∈ Finset.range N, ((ZMod.χ₈ n : ℤ) : ℝ) / (n : ℝ)) atTop
      (𝓝 (Real.log (1 + Real.sqrt 2) / Real.sqrt 2 : ℝ)) := by
  convert chi8Partial_tendsto using 1
  funext N
  rw [chi8_series_partial_real]

lemma chi8ℂ_term_eq (n : ℕ) :
    (algebraMap ℝ ℂ) (((ZMod.χ₈ n : ℤ) : ℝ) / (n : ℝ)) = (χ₈ℂ n) / (n : ℂ) := by
  have hnum : (algebraMap ℝ ℂ) (((ZMod.χ₈ n : ℤ) : ℝ)) = ((ZMod.χ₈ n : ℤ) : ℂ) := by norm_num
  have hnum2 : ((ZMod.χ₈ n : ℤ) : ℂ) = (χ₈ℂ n) := by simp [χ₈ℂ]
  have hden : (algebraMap ℝ ℂ) (n : ℝ) = (n : ℂ) := by rfl
  ...
  calc
    (algebraMap ℝ ℂ) (∑ n ∈ Finset.range N, (((ZMod.χ₈ n : ℤ) : ℝ) / (n : ℝ)))
        = ∑ n ∈ Finset.range N, (algebraMap ℝ ℂ) (((ZMod.χ₈ n : ℤ) : ℝ) / (n : ℝ)) :=
          map_sum (algebraMap ℝ ℂ) (fun n : ℕ => ((ZMod.χ₈ n : ℤ) : ℝ) / (n : ℝ)) (Finset.range N)
    _ = ∑ n ∈ Finset.range N, (χ₈ℂ n) / (n : ℂ) :=
          Finset.sum_congr rfl (by intro n hn; exact chi8ℂ_term_eq n)

/-- The `L(1, χ₈) = ln(1+√2)/√2` special value as a Dirichlet-series limit: the ordered partial
sums `Σ_{n<N} χ₈(n)/n` (in `ℂ`) tend to `ln(1+√2)/√2`. -/
lemma chi8_series_ln_one_plus_sqrt_two_div_sqrt_two :
    Tendsto (fun N : ℕ => ∑ n ∈ Finset.range N, (χ₈ℂ n) / (n : ℂ)) atTop
      (𝓝 (Real.log (1 + Real.sqrt 2) / Real.sqrt 2 : ℂ)) := by
  have hc : Tendsto (RCLike.ofReal ∘ (fun N : ℕ => ∑ n ∈ Finset.range N, ((ZMod.χ₈ n : ℤ) : ℝ) / (n : ℝ))) atTop
      (𝓝 (RCLike.ofReal (K := ℂ) (Real.log (1 + Real.sqrt 2) / Real.sqrt 2 : ℝ))) :=
    RCLike.continuous_ofReal.continuousAt.tendsto.comp chi8_series_real_ln_one_plus_sqrt_two_div_sqrt_two
  have hpi : 𝓝 (RCLike.ofReal (K := ℂ) (Real.log (1 + Real.sqrt 2) / Real.sqrt 2 : ℝ)) =
      𝓝 (Real.log (1 + Real.sqrt 2) / Real.sqrt 2 : ℂ) := by
    congr 1
    change (algebraMap ℝ ℂ) (Real.log (1 + Real.sqrt 2) / Real.sqrt 2) =
      (Real.log (1 + Real.sqrt 2) : ℂ) / (Real.sqrt 2 : ℂ)
    rw [map_div₀]
    simp
  have hobs : ∀ᶠ N in atTop,
      (RCLike.ofReal ∘ (fun N : ℕ => ∑ n ∈ Finset.range N, ((ZMod.χ₈ n : ℤ) : ℝ) / (n : ℝ))) N =
