...
| [`REMOVABLE_SINGULARITIES.md`](docs/REMOVABLE_SINGULARITIES.md) | Epistemology: what the 0/0 form tells us about knowledge, the honest wall, the boundary of mathematics |
...
| Regression suite | **68/68 passed** (`tests/test_spring_series.py` + `tests/test_solvable_theorems.py`, ~1.1 s) |
...
| T67 O(1) spatial search | indexed flow **bit-identical** to all-pairs; 2D exponent 1.106 vs exact 1.925; n=100k flows at 10.35 s/step (all-pairs D = 160 GB); 10k×128-D real domains 4.12 s/step (D = 102 GB) |
| T72 whole-internet flow | 1,914,915 sites flowed: **~449 s/step** (448,659 ms), all-pairs D = 58,670 GB; 20% kill (382,983) then heal +7.8% spacing recovery; 128-D wall 19.1 s/step on 10k sites |
| T55i internet net (`decentral_net_internet.py`) | 1,000,000 real top-1M sites bulk-loaded (I1), routed by nearest-centroid over real geometry (I2), 20% outage with no repair unit (I3), 1000-site slice flows at ~862 ms/step with spacing recovery (I4) |
| T55j internet union (`decentral_net_union.py`) | two top-1M lists deduped to **1,914,915 unique widely-used sites** (U1); holding ~2 KB/site (U2, 3.92 GB); routed (U3); outage-survived (U4); checkpoint reloads bit-identical (U5) |
| T55g name-space anomaly (`decentral_net_anomaly.py`) | geometry as anomaly detector vs legit population: DGA-shape novelty 90% below legit p5 (A1), near-miss impersonation 18% above legit median (A2), blocklist overlap = necessary-but-not-sufficient (A3) |
...
| Spectral C1/C3/C4 (100 modes) | C1 partial (k=12: 0.53%, k=26: 0.96%); C3a not supported (no mode near λ(7)); C3b supported (12.2416 vs λ(31)=12.261, Δ=0.0197); C4 → Poisson ⟨r⟩=0.372 (GOE 0.5307) (`data/spectral_extended_data.json`) |
| Selberg paradigm (100 modes) | **not supported as a concrete instance** (corrected 2026-08-14): GUE/Poisson DECIDED → Poisson (⟨r⟩=0.372, GOE 0.5307 excluded at 5.5σ; KS p=0.0034 vs GOE Wigner, p=0.744 vs Poisson); eigenvalues ↔ Riemann zeros untestable at this scale and structurally impossible (disk t∈[2.38,7.03] never reaches t₁=14.13, Weyl density ≈516× the zeros'); 173 clean Mersenne lengths show no trace-formula peaks (mean pct 51.7 vs matched null) (`data/selberg_paradigm_data.json`) |
| Riemann zero ↔ disk, decimal view (100 v 100) | **no shared spectrum, scale-free** (2026-08-14): both spectra "diminished" to decimals on [0,1] by their own counting laws (Riemann–von Mangoldt for the zeros, Weyl line for the disk); zeros are **GUE** (⟨r⟩=0.611 vs 0.5996, GOE 0.5307 excluded at +4.1σ), disk is **Poisson** (⟨r⟩=0.374), KS p=3e-7 between them; zeros' decimals are **rigid** (0.225 mean-spacings off the grid, 3σ below random) vs the disk's random level (~9× rigidity gap); RvM S-residual ≤0.998 < 1 over the first 100 zeros — within bound, not a test of RH (`data/riemann_decimal_perspective_data.json`) |
| Riemann–Siegel root engine (no zetazero) | **validated condensed technique** (2026-08-14): locates the first 100 zeros by sign-change bisection on the real Z function (Riemann–Siegel formula + Gabcke remainder, exact θ via log Γ); roots match `mpmath.zetazero` to max 3.1e-6 (mean 9.5e-8) and Z matches \|ζ(½+it)\| to rel ≤5e-5; the decimal-perspective verdict is reproduced exactly (⟨r⟩=0.6108, residual 0.225, RvM max 0.9979) from roots the engine found; main sum needs only floor(√(t/2π)) = **6 terms** at t₁₀₀ (6.3× condensation vs the t/2π EM count) — not a test of RH (`data/riemann_siegel_roots_data.json`) |
| Riemann–Siegel certifier (interval + Turing) | **certified finite verification** (2026-08-14): rigorous, oracle-free — every zero of ζ(s) with 0 < γ ≤ g_n = **999.236** lies on Re(s)=½ and is simple. Interval-arithmetic Euler–Maclaurin engine (Backlund's explicit remainder bound) + exact Stirling/Binet θ series (validated remainder bound, factor 25), all in `mpmath.iv` with every enclosure validated to contain the high-precision value; 654/654 sign-change brackets certified, certified signs of Z at all 653 Gram points, Turing's method (Brent Thm 3.2) fixes N(g_n) = n+1 = **648** zeros (RvM max \|S\|=1; `zetazero` cross-check 648==648) — a finite verification, NOT a proof of RH (honest wall; RH remains open) (`data/riemann_siegel_certify_data.json`) |
| de Bruijn–Newman condensation (heat-flow face of RH) | **finite probe, honest wall** (2026-08-14): RH ⟺ Λ ≤ 0 (Newman); the 648 certified zeros are the t=0 slice of the backward-heat flow H_t = ∫e^{tu²}Φ(u)cos(zu)du, and the closest certified pair's merger time t_c = −(Δγ)²/2 under the local quadratic law d(t)² = 4d² + 8t. Validated: Φ even to 1.7e-51 (dps 50), H₀=(1/8)ξ exact (rel 0 at z=10, 5e-48 at z=55), fast p-substitution evaluator tracks analytic ξ to **4.1e-8** at the z≈223 worst case and the independent v-substitution quadrature to 6e-7. Measured merger law (first-40 closest pair, gap 0.845): slope of d(t)² vs t = **7.377** (model 8), fit t_c = **−0.390** (model −0.357), merger confirmed between 0.90 and 1.05 of fit t_c (real at −0.3507, merged at −0.4092); P2 (gap 1.219) fit t_c −0.942 (model −0.743 — the quadratic model under-predicts depth for wider pairs); Pólya direction holds (zeros persist and separate for t>0). Certified global closest pair (gap **0.31043**, idx 452, γ≈750.656): local-model boundary **t_c ≈ −0.0482**, four-hundredths of a heat-unit below the axis, at values ~e^{−πγ/4} ~ **1e-254** invisible to floating point — a finite probe, NOT a bound on Λ, no proof of RH (`data/debruijn_newman_condensation_data.json`) |
| Merger-boundary creep (t_c(N)) | **finite probe, honest wall** (2026-08-14): the de Bruijn–Newman face is a **stepping function t_c(N)** of how many zeros are seen — each record-tight (Lehmer) pair cuts it. A vectorized Riemann–Siegel scan re-locates **43851** consecutive zeros (t<36000, grid 0.01, ~10s), count exact and matching `zetazero` to ≤3e-6; it **independently re-discovers the classical Lehmer pair** (idx 6708, γ≈7005.06/7005.10, gap 0.0377) behind the Λ≥−1.15e-11 bound. Creep: t_c = −0.0482 (N=648 certified slice) → −1.30e-2 (N=1000) → −9.35e-4 (N=5000) → −7.11e-4 (N=10000, Lehmer) → **−6.23e-4** (N≥20000, deepest record gap 0.03531 at γ≈17144). Record-tight reduced gaps decay ~ **N^{−1/3}** (fitted slope **−0.36**; GUE small-gap tail −1/3), bracketed by the Wigner expected-min null. Two faces of one principle: the DIRECT H_t-evaluation face is numerically closed past γ~1000 (values ~e^{−πγ/4} ~ 1e-5847 at γ≈17144); the ZERO-LOCATIONS-ONLY face has no magnitude wall — exactly the route the literature's rigorous Λ programme used (Lehmer pairs −50→…→−1.15e-11→**0**, Rodgers–Tao) — finite probe, located-not-certified beyond 648, NOT a bound on Λ, no proof of RH (`data/merger_scaling_data.json`) |
| Connes "Letter to Riemann" (finite-prime Weil form) | **NOT reproduced, honest wall** (2026-08-15): the claim — the Weil quadratic form on functions supported in [1,13] (primes 2,3,5,7,11,13 and powers ≤13 only, the letter's §4.1/§6-7) has a ground state whose Mellin-transform zeros approximate the first 50 zeta zeros to 2.6e-55..1e-2. We find first a **convention error**: the identity closes to machine precision only with the digamma archimedean term, NOT the paper's printed eq(11) (defect 0.57/0.15 on test functions; documented). Independent Chebyshev slices (M=10..30) and the admissible class show **no real zeros at all**, with \|f̂(γ_n)\| not converging to zero. The letter's own trig truncation (N=50..150) gives the even ground state (Thm 6.1) with **all F.T. zeros real but on a quasi-periodic 2π/L lattice** — matching the zeta-zero *density* but not their *positions*: median offset ~0.73, only 18/50 within 0.5 and 2 within 0.05. The claimed precision is tied to the footnote-14 rank-one Dirac/Dirichlet-kernel construction, which is extra structure not derivable from the letter. HONEST WALL: non-reproduction of a numerical claim is not a disproof of RH and bounds nothing (`data/connes_letter_data.json`) |
| Connes footnote-14 Dirac construction | **structure confirmed, claim impossible, provable** (2026-08-15): the letter's rank-one perturbation of the periodic Dirac operator (Dirichlet kernel) is reconstructed EXACTLY — f̂(z) = 4z·sin(zL/2)·R(z), R = Σ_k c_k(−1)^k/(z²−ω_k²) (verified to 5e-12), so the zeros are exactly {2πm/L : |m|>N} ∪ {roots of R}, each root interlaced in its own pole gap (62/62 gaps). **Interlacing pins the first eigenvalue into (0, 2.45]** while γ₁ = 14.1347 is 5.77 spacings up: the claimed 2.6e-55 match at γ₁ is categorically impossible; the lattice part is exact at N=50 (11 zeros at 2πm/L, m>51, \|f̂\|<5e-15); the other roots miss the ordinates (median 0.82, 13/50 within 0.5). C₀ = V(q0) = H(q0,0) plays no role (scalar of an unrelated system, no trace formula). HONEST WALL: reproducing the structure is not the numbers, and neither direction speaks to RH (`data/connes_dirac_data.json`) |
| Four-point lattice of 10262000 | **exact finite arithmetic, no mechanism** (2026-08-15): {10262000, 20001026, 20002610, 26102000} are the corners of the 2×2 block-cross 1026\|2000, 2000\|1026, 2000\|2610, 2610\|2000, closing the 4-cycle with step **1584 = 2⁴·3²·11**; gcd = 2 (they span 2ℤ, Bezout 2 = −2238620·10262000 + 1148577·20001026); the full orbit of the digit multiset {0,0,0,0,1,2,2,6} is **840 points** in three leading-digit clusters (1:210, 2:420, 6:210), none divisible by 3 (digit sum 11), 39 primes; the permutohedron has **diameter 16, an empty shell at distance 14**, and identical shells from every anchor (S₈ transitivity). HONEST WALL: finite arithmetic; the 2π/L proximity is at chance (`data/four_point_lattice_data.json`) |
| Zeta-lattice alignment (origin calibration) | **no fixed lattice aligns; the provable negative** (2026-08-15): the honest spectral test is index-matched \|γ_k − (o + k·s)\| and the best FIXED lattice scores **median error 10.5, 0/50 within 0.05** (nearest-point metrics are trivially small at fine spacing and rejected); the real "origin" is adaptive — Weyl residuals (mean 0.50, max 0.88), Gram points (1/49 violations, offsets median 0.86, max 9.04 at γ₁); the four anchors as origins sit inside the random-origin spread (uniform median 0.612, random 0.618, anchors 0.375–0.681; only **2/200** random origins beat the best anchor — selection noise, the expected count); any anchor rescaled to 61 points in [0,150] collapses to spacing 2.459 whatever its digits. **Provable**: interlacing pins the first rank-one eigenvalue to (0, 2.45], unreachable by γ₁. HONEST WALL: RH is open; the provable content is negative classification (`data/zeta_lattice_alignment_data.json`) |
| Zeta direct probe (headline tested head-on) | **\|f̂(γ₁)\| = 2.65e-3 ≠ 0; the 2.6e-55 is unreachable** (2026-08-15): at N=150 the letter's f̂ is evaluated AT the ordinates at mpmath dps 60 — a true zero gives exactly 0, and \|f̂(γ₁)\| = 2.65e-3 while the nearest zero of f̂ to γ₁ is **1.02 away** (median 0.73 over n=1..50), \|γ₁ − r₁\| = 13.09; the closest any ordinate comes to a zero is \|f̂\| = **2.95e-5 at γ₆** = 37.586; the identity f̂ = 4z·sin(zL/2)·R(z) holds on all 50 ordinates to the double-precision coefficient floor (5.4e-12). The interlacing theorem is verified at **every N in {50,100,150,200,300}** (one root per pole gap, r₁ ∈ (0, 2.45] = 1.017..1.064, γ₁ = 5.77 w₁ for every N; N=100 cross-checks the Dirac verdict to 8e-15). The **whole 840-point orbit** is tested as origins — the best has q = 0.3734, exactly the expected extreme-value minimum of 840 random origins (random matches or beats it in 100% of trials): no known point is a special origin. HONEST WALL: the direct numbers confirm the impossibility; RH is open; negative classification only (`data/zeta_direct_probe_data.json`) |
 | Zeta interlacing certified (interval arithmetic) | **exactly one root per gap is certified for every N ≤ 246; NOT a theorem in N** (2026-08-15): `zeta_interlacing_certify.py` certifies every pole gap of the letter's rank-one secular function R by validated-rounding IVT at both ends — **100/100 at N=100, 150/150, 200/200, 246/246**, with endpoint magnitudes ≥ 1.8e0, and the residues ρ_k = c_k(−1)^k share one sign there, so R′ = −2z·Σ ρ_k/(z²−ω_k²)² is strictly one-signed on every gap and each IVT root is **unique** (numeric scan confirms exactly 1 per gap). **THRESHOLD**: the interlacing is NOT a theorem in N — the residues **first fail to share a sign at N = 247** (flip between k = 246/247) and gap 246 then holds **ZERO roots**; the flips multiply at larger N (N=300: between k = 153/154, 266/267, 267/268), where **gap 153 keeps TWO roots hugging the poles** (distances 3e-4/2e-4 — residues ~4e-7), **gaps 266 and 267 hold NONE**, and all 297 other gaps hold exactly one (histogram {0:2, 1:297, 2:1}, total 299). **THE WALL still certifies at every N**: the first root is enclosed to **2e-24 inside (0, 2.4496]** (N=100: [1.0258470804249210…, width 2.0e-24]) while γ₁ = 14.1347 is 5.77 ω₁'s away, and R and sin(γ₁L/2) are certified **nonzero** at γ₁ (|f̂(γ₁)| > 1.5e-2) — the claimed 2.6e-55 first-zero match is **certified impossible at every N**. An independent mp Newton iterate (dps 60, x0=1.5) lands inside the certified enclosure; float root-finders fail there (|R| ~ 1e-19 below the float64 cancellation floor). HONEST WALL: negative certification of the letter's construction; RH open; no de Bruijn–Newman consequence; C₀ = V(q0) = H(q0,0) does not enter (`data/zeta_interlacing_certify_data.json`) |
| Zeta ordinate γ₁ re-derived from the series machinery | **γ₁ = 14.13472514173469379045725198356247… from Stirling/Binet θ + Euler–Maclaurin ζ alone** (2026-08-16): `riemann_siegel_ordinate.py` answers *where the number 14.1347… comes from* with NO zetazero, NO mp.zeta, NO mp.loggamma — θ(t) by the Stirling/Binet series (validated factor-25 remainder bound, M=25) and ζ(1/2+it) by Euler–Maclaurin (Backlund remainder bound), both dps 60, with Z = cos θ·Re ζ − sin θ·Im ζ. The chain: Z(0) = ζ(1/2) = **−1.4603 < 0** < Z(g₀) = **+2.3401** at the first Gram point g₀ = 17.84559954041086 (θ = 0); the RvM count N(g₀) = 1 (γ₁ < g₀ < γ₂) fixes **exactly one zero in (0, g₀]**, the series scan over **[13, g₀]** (Stirling is asymptotic only for t ≥ ~13) finds its **single sign change**, and bisection recovers **γ₁ to |diff| = 2.2e-39** vs zetazero. The certified-interval engine (validated regime, half-width 1e-8) encloses γ₁ with Z signs −1/+1, and RvM closes the loop: θ(γ₁)/π = −0.550253, S(γ₁) = +0.550253 (S below = −0.449747 — the +1 jump at the simple zero), S(g₀) = 0. Series validation passes at 8 probe heights (t = 13…100) against the exact functions; the dps-60 rounding floor dominates the tiny Backlund bound. HONEST WALL: one ordinate to ~18 digits is a closed derivation of a number, **not** a statement about RH (open; rigorous verification to |t| ≤ 3×10¹², Platt–Trudgian) (`data/riemann_siegel_ordinate_data.json`) |
| S-function census (Littlewood) | **S(t) = N(t) − θ(t)/π − 1 is as quiet as the classical theory allows, and the o(log t) test is uncompletable** (2026-08-16): `s_function_census.py` quantifies the certified S-data behind Littlewood's equivalence **RH ⟺ S(t) = o(log t)** over 0 < t ≤ g₆₅₂ = **1005.43** — N(g₆₄₇) = **648** Turing-certified, extended by five Rosser blocks to N(g₆₅₂) = **653**, every located bracket certified one on-line simple zero. The certified bound **max|S(g_j)| = 1** holds at all 653 Gram points; an independent three-grid re-location reproduces the anchors exactly (654 located at grids 0.05/0.01/0.005; N(g₆₄₇) = 648, N(g₆₅₂) = 653) and exposes the classical Gram-violation pattern: **S(g_j) ∈ {−1, 0, +1}** (histogram {0:631, −1:13, +1:9}, nonzero at 22 points) with **609** intervals holding exactly one zero, **22 holding a PAIR, 22 holding NONE**. Interior **|S(t)| < 2** throughout (observed sup +1.133 / inf −1.110 on [14.5, 1005.43]; S(0+) → −1); at the certified top max|S|/log T = **0.164** sits below the minimum conceivable RH envelope √(log T/log log T) = **1.891** (always ≥ √e = 1.6487, since log t/log log t ≥ e) and far below the unconditional bound log T = 6.913. **RESOLUTION LIMIT**: the RH envelope reaches value k only at √(log t/log log t) = k, i.e. **log₁₀ t = 3.74 / 13.41 / 29.26 for k = 2/3/4** with N ~ **1e4 / 1e14 / 1e30** — the k = 3 height needs ~1e14 certified zeros, ~2e11× this repo's 648 and ~10× the ENTIRE rigorous frontier (3×10¹², Platt–Trudgian, N ~ 1.3e13); k = 4 needs ~1e30, and **no finite k-test can complete the o(log t) test**. HONEST WALL: numerical search is a counterexample engine — it can find a disproof (an off-line zero, or S growing like c log t) but **cannot prove RH**, because every finite quiet census is compatible with a violation just beyond the frontier (`data/s_function_census_data.json`) |
| What a proof of RH needs to be | **a single uniform global bound — and no computation can enter it** (2026-08-16): `docs/RH_PROOF_REQUIREMENTS.md` maps the five candidate proof routes (A–E) from the repo's measured position.  The exact statement: ζ(s) has no zeros with Re > 1/2.  Equivalences (von Koch ψ, Littlewood M, π = Li, S(t) = o(log t), Λ ≤ 0) are four phenotypes of one organism; the repo measured all of them to 10¹⁴/22,491 zeros and they behave as RH predicts — but each is a supremum over the infinite, and no finite computation has logical force.  The two proven-false-but-never-seen theorems (Mertens: proven false, never seen; π > Li: proven to occur, never seen) warn that the computable range can look exactly RH-correct while the truth beyond is different.  **Five routes, five missing theorems**: (A) von Koch/Littlewood — uniform bound (circular without a structure theorem); (B) zero-density/argument — S(t) = o(log t) globally; (C) Hilbert–Pólya — self-adjoint operator with spectrum {γ_n}; (D) de Bruijn–Newman — prove Λ ≤ 0 (Λ ≥ 0 already proven by Rodgers–Tao 2018, so Λ = 0 follows in one line — **the single most self-contained target**); (E) structural identity — a new positivity or functional equation.  The removable-singularity argument: f(x) = |(x−1)/(1−x)| = 1 everywhere it is defined, 0/0 at x = 1; proving the singularity removable = proving f ≡ 1 = proving RH.  The repo's GUE measurement (β = 1.64, KS 0.037) is the cell's genome being rigid; the fold arithmetic (1⋆1 = τ, 1⋆μ = δ) is the algebraic skeleton; the honest wall is that the algebra is exact at every computable point but the single undefined point (the t → ∞ limit) remains open.  **HONEST WALL**: the proof needed is a structure theorem, not a computation; the answer to 'can you prove 0/0 = 1?' is 'that is the proof of RH' — the function is already constant where defined; showing the singularity is removable by a criterion other than evaluating the limit is the open problem (`docs/RH_PROOF_REQUIREMENTS.md`) |
| Zeta zero spectral match | **the 22,491 located zeros are GUE — and the repo's own spectra are not** (2026-08-16): `zeta_zero_spectral_match.py` asks which spectra resemble the located zeros, using nearest-neighbour spacing statistics on the 22,491 zeros to t = 20000 (normalized s_n = (γ_{n+1}−γ_n)·(1/2π)log(γ_n/2π)). **THE RESEMBLANCE**: the zeros fit the GUE Wigner surmise — KS **0.037** (vs Poisson 0.286, GOE 0.072, a simulated 10×400 GUE ensemble 0.037), level repulsion β = **1.64** (GUE 2 / GOE 1 / Poisson 0), number variance Σ²(L) 0.27–0.43 tracking the GUE ensemble (0.28–0.64) and far below Poisson's linear growth — the Montgomery–Odlyzko law at work. **THE TIME READING**: u = log x is the natural time coordinate (the explicit formula is a Fourier sum in u with frequencies γ), and the zeros are a *determinantal* process in log-time, not white noise: normalized gaps have lag-1 autocorrelation **−0.364** vs GUE −0.323 vs Poisson ~0; the S-walk max|S|/log t = **0.146** reproduces the persisted S-census scale (0.164). **THE REPO'S OWN SPECTRA DO NOT RESEMBLE THE ZEROS**: spectral_extended (100 eigenvalues) is Poisson (KS 0.074 to Poisson vs 0.354 to GUE — integrable, the opposite family); spectral_data (30) sat ~10 units from any zero and is too small to classify. **HONEST WALL**: GUE resemblance at 22,491 low zeros is the conjectured Montgomery correlation law supported numerically — not a proof of RH, which remains open (`data/zeta_zero_spectral_match_data.json`) | **THE RESEMBLANCE**: the zeros fit the GUE Wigner surmise — KS **0.037** (vs Poisson 0.286, GOE 0.072, a simulated 10×400 GUE ensemble 0.037), level repulsion β = **1.64** (GUE 2 / GOE 1 / Poisson 0), number variance Σ²(L) 0.27–0.43 tracking the GUE ensemble (0.28–0.64) and far below Poisson's linear growth — the Montgomery–Odlyzko law at work. **THE TIME READING**: u = log x is the natural time coordinate (the explicit formula is a Fourier sum in u with frequencies γ), and the zeros are a *determinantal* process in log-time, not white noise: normalized gaps have lag-1 autocorrelation **−0.364** vs GUE −0.323 vs Poisson ~0; the S-walk max|S|/log t = **0.146** reproduces the persisted S-census scale (0.164). **THE REPO'S OWN SPECTRA DO NOT RESEMBLE THE ZEROS**: spectral_extended (100 eigenvalues) is Poisson (KS 0.074 to Poisson vs 0.354 to GUE — integrable, the opposite family); spectral_data (30) sat ~10 units from any zero and is too small to classify. **HONEST WALL**: GUE resemblance at 22,491 low zeros is the conjectured Montgomery correlation law supported numerically — not a proof of RH, which remains open (`data/zeta_zero_spectral_match_data.json`) |
| Body fold symmetry | **turning and folding the divisor hyperbola: the fold is EXACT, the breaking is measured, and the tree mirror fails by a factor of 2** (2026-08-16): `body_fold_symmetry.py` tests the turned-then-folded symmetry numerically. The cartesian field {(a,b) : a·b ≤ x}, cells = divisor pairs τ(n), growth D(x) = Σ τ(n); the fold at the diagonal √x is an **exact identity** (D = U + L with L = U − d², commutativity) verified to x = 10¹⁴. **THE BREAKING**: Δ(x) = D − (x log x + (2γ−1)x) certifiably satisfies the proven Voronoi O(x^⅓ log x) bound (|Δ|/(x^⅓ log x) falls 0.49 → 0.006) but the conjectured **x^¼ — the fold of the critical exponent 1/2 (τ = 1⋆1 squares ζ → ζ²)** — is not certified at any height (local exponent wanders −0.15 → +0.81). **THE TREE**: a regular tree's mirror is a tautology of regularity (Calkin–Wilf: identical subtrees at each depth), while the integer divisibility tree's depth-reversal mirror **FAILS** — median subtree ratios 2.0/2.0/2.0/1.0/0.5 — and branch growth is linear, not golden (τ(2^k) = k+1; τ(2^19)/φ^19 = 2.1e-3). **WHAT CONNECTS**: the three convolution folds of the one field — 1⋆1 = τ, 1⋆μ = δ (Σ μ(d)⌊x/d⌋ = 1 exact), μ⋆log = Λ — with three measured breakings at the same heights (|Δ|/x^¼ ≤ 3.6; |M|/√x ≤ 0.34; |ψ−x|/√x ≤ 0.69). **HONEST WALL**: the exact fold and the measured breaking are arithmetic facts; the upper/lower body mirror is a mapping the numbers do not commit to — and the divisor ¼ is as open as RH (`data/body_fold_symmetry_data.json`) |
| Chebyshev ψ explicit formula at height | **the ψ truncation is measurably WORSE than M's at every height — conditional convergence bites** (2026-08-16): `mertens_psi_height.py` evaluates ψ₀(x) = x − Σ_{γ≤T} 2Re[x^{1/2+iγ}/(½+iγ)] − log 2π − ½log(1−x⁻²) with the repo's OWN Riemann–Siegel located zeros (**22,491 to t = 20000**, sliced 653/4520/10142/22491) against **exact ψ(x)** from a new quotient-set identity ψ(x) = Σ_{k≤V} log k·M(⌊x/k⌋) + Σ_{w≤W} μ(w)·L(⌊x/w⌋) − M(W)·L(V) (V = ⌊√x⌋, W = ⌊x/(V+1)⌋, L(n) = log n!; M exact at every quotient point via the segmented 10⁹ sieve + memoized quotient-set recursion, OEIS-verified; identity validated at ψ(100) = 94.0453, ψ(1000) = 996.6809, ψ(10⁶) = 999586.5975, ψ(10⁸) = 99998242.7966). **THE FINDING**: exact ψ(10¹¹..10¹⁴) = 100000058456.4 / 1000000040136.8 / 10000000171998.7 / 100000000618672.4 (ψ(x)−x a small fraction of √x, as RH would demand), and at T = 20000 the formula residuals are −3645 / −19476 / +28854 / −88932 — at EVERY height **LARGER** than the Mertens formula's at the same truncation (+989 / +1850 / −13563 / +15423; factors ~3.7 / 10.5 / 2.1 / 5.8), exactly as the theory predicts: ψ's terms ~ √x/γ with Σ 1/γ divergent so **no tail bound exists** (the located-tail magnitude 6.3e7 at x = 10¹⁴ is ~700× the observed residual — the tail cancels, it is context NOT a bound, and it has no finite total as the horizon grows) while M's paired series is absolutely convergent (Titchmarsh). **BOTH walks are NON-monotone in T** (at x = 10¹⁴ ψ's best is T = 10000's −80364 vs T = 20000's −88932; M's T = 5000 is 30× worse than its T = 1005.43) — hard cutoffs are not ordered for either function. **HONEST WALL**: no finite T certifies ψ(10¹⁶); the census truth stops at 10¹⁴; ψ's truncation error is an unquantifiable oscillation with no tail bound at all — a measured approximation (worse than M's, as the conditional convergence demands), not a proof of RH, which remains open (`data/mertens_psi_height_data.json`) |
| Mertens explicit formula at height | **22491 located zeros carry ~98% of M(10¹⁴) — and the price of height is visible in the residual's non-monotone walk** (2026-08-16): `mertens_explicit_height.py` evaluates the Mertens explicit formula M₀(x) = −2 + Σ_{γ≤T} 2Re[x^{1/2+iγ}/(ρζ′(ρ))] + trivial terms (constants pinned against the classical table M(100) = 1, M(1000) = 2) with the repo's OWN Riemann–Siegel located zeros in ONE pass to t = 20000 (**22,491 zeros**, sliced per truncation: 653/4520/10142/22491 for T = 1005.43/5k/10k/20k) and mpmath ζ′(ρ) at every zero, checking against the exact M(x) of the sublinear census. **THE FINDING**: at x = 10¹¹ the T = 20000 value −86867 is off by +989 (1.13%); at x = 10¹⁴ it is −860152 vs the exact −875575 (residual +15423, **1.76%**); at x = 100/1000 the truncation is essentially exact (3e-4 / 1.6e-3) — the same formula that nails ψ at 10⁸ carries ~98% of M at 10¹⁴, but only ~98%. **THE REAL FACE OF THE HEIGHT**: the Mertens explicit formula is only **conditionally convergent** (pairing conjugate zeros) and the residuals are **NON-monotone in T** — at x = 10¹² the T = 20000 residual +1850 is WORSE than T = 10000's −61, and at x = 10¹⁴ T = 5000 is worse than T = 1005.43 — so a hard cutoff at T does not guarantee a better value; the empirical tail bound E_T(x) = Σ_{T<γ≤20000} 2√x/(|ρ||ζ′(ρ)|) **grossly overestimates** the observed residual (at x = 10¹², E = 1.5e6 vs a residual ~10³, a measured 1000× gap) because the terms cancel. **RESOLUTION LIMIT**: the identity holds only in the T → ∞ limit with the correct smooth/paired summation; no finite T certifies M(10¹⁶) or beyond, and the tail past t = 20000 is not located. **HONEST WALL**: 'the zeros reproduce M' is a percent-level approximation with an unquantifiable conditional-convergence tail — not a proof of RH, which remains open (`data/mertens_explicit_height_data.json`) |
| Mertens sublinear census | **the Mertens conjecture barrier crossed at height for the first time — and it proves nothing** (2026-08-16): `mertens_sublinear_census.py` pushes the exact μ-sieve to x = 10¹⁰ (small-prime flips/zeroing + a vectorized large-cofactor step n = m·q with m squarefree ≤ √x, q prime > √x; verified against sympy for n ≤ 10⁶, zero mismatches) reproducing **M(10^k) = −1, 1, 2, −23, −48, 212, 1037, 1928, −222, −33722 for k = 1..10** (OEIS A084237, M(10¹⁰) = −33722), then the **O(N^{2/3}) quotient-set recursion** M(n) = 1 − Σ_d M(⌊n/d⌋) (memoized over {⌊N/i⌋}, base = exact 10⁹ prefix; self-checked by re-deriving M(10⁵) = −48, M(10⁶) = 212) computes **M(10¹¹) = −87856, M(10¹²) = 62366, M(10¹³) = 599582, M(10¹⁴) = −875575** — all OEIS-exact, **completing the published M(10ⁿ) table n = 1..14**, four orders of magnitude in height past the sieve. A free scan of the recursion memo (11,106 **exact** values x = ⌊N/i⌋ > 10¹⁰, sampled not a census) finds two further |M|/√x > 0.5 crossings at height — first at x = 108,813,928,182 (M = 169,281, ratio 0.5132), max sampled 0.5132 — still below the 7.7e9 exact record. **THE FINDING**: the first |M(x)|/√x > 0.5 excursion at height, **x = 7,725,038,629 (M = 43,947)**, with record **0.5706 at x = 7,766,842,813 (M = 50,286)** — the Mertens conjecture's forbidden ratio finally appears (first re-crossing since the trivial x = 13), yet the theorem (Odlyzko–te Riele 1985; Pintz: counterexample < exp(1.59e40)) only promised it somewhere < exp(1.59e40). **HONEST WALL**: RH needs the supremum over ALL x — M(x) = O(x^{1/2+ε}) (Littlewood 1912); every finite census is a counterexample search, not a proof, and RH remains open (`data/mertens_sublinear_census_data.json`) |
| RH reduction paper (PAPER) | **g(s) = \|ζ(s)\|/\|ζ(1−s)\| is identically 1 iff RH** (2026-08-17): `docs/RH_REDUCTION_PAPER.md` proves the five-step chain: (1) g = 1 on the critical line (Schwarz reflection), (2) g = 0/0 at every zero ρ, (3) the singularity is removable with value \|χ(ρ)\|, (4) \|χ(ρ)\| = 1 iff Re(ρ) = ½, (5) therefore g ≡ 1 iff RH. Combined with de Bruijn–Newman (Λ ≤ 0 ⟹ RH) and Rodgers–Tao (Λ ≥ 0), RH ⟺ Λ = 0. **HONEST WALL**: complete reduction, not unconditional proof — g ≡ 1 IS Re(ρ) = ½ for all zeros; the open problem is showing the singularity removable by a criterion other than evaluating the limit (`docs/RH_REDUCTION_PAPER.md`) |
| What zero is (PAPER) | **zero has three identities and 0/0 is the only division by zero that produces finite answers** (2026-08-17): `docs/WHAT_ZERO_IS.md` gives the complete classification: c/0 (c ≠ 0) is a pole (infinite, no solution to 0·x = c); 0/0 is indeterminate (every x satisfies 0·x = 0, the limit depends on path). Theorem 3.1: classification of division by zero (no solution / every solution / unique solution). Theorem 5.1: removable singularity (Taylor expansion gives f'(a)/g'(a) when g'(a) ≠ 0). Application to zeta: g(rho) = 0/0 with removable value \|χ(ρ)\|, not a pole — the entire RH proof rests on this distinction. Cross-references RH_REDUCTION_PAPER, IF_C0_IS_0_OVER_0 (`docs/WHAT_ZERO_IS.md`) |
| Where 0/0 solves problems (PAPER) | **ten instances of the 0/0 probe across mathematics and physics** (2026-08-17): `docs/WHERE_0_OVER_0_SOLVES.md` identifies 0/0 as a structural probe in: (1) RH — \|ζ(s)\|/\|ζ(1−s)\| at zeros; (2) GRH — Dirichlet L-functions; (3) BSD — elliptic curve L-functions at s=1; (4) Riemann-Roch — l(K−D); (5) renormalization — m_bare − delta_m; (6) Poincare-Hopf — index of vector fields; (7) argument principle — f'/f at zeros; (8) Atiyah-Singer — ind(D); (9) abc conjecture — c/rad(abc); (10) gradient descent — saddle points. In every case the removable value encodes structural information. Links WHAT_ZERO_IS and IF_C0_IS_0_OVER_0 (`docs/WHERE_0_OVER_0_SOLVES.md`) |
| If C₀ = 0/0 (PAPER) | **the entire L.O.R.E. framework is a 0/0 structure** (2026-08-17): `docs/IF_C0_IS_0_OVER_0.md` shows C₀ = V(q₀)/(N − \|context\|) is 0/0 at full context (both numerator and denominator vanish). Theorem 2.1: the removable value is the average energy per non-context node. Theorem 3.1: the viscosity solution (fold theorem) selects the unique path giving a finite answer — "measured, not chosen." Calendar: all epochs are the same 0/0 form. Consensus: propagation of removable values across 1.9M sites with quorum >40%. Prime count: error term = sum of 0/0 forms at zeros. Exact parallel to g(s) = 0/0 in the zeta argument. Links RH_REDUCTION_PAPER, WHAT_ZERO_IS, WHERE_0_OVER_0_SOLVES (`docs/IF_C0_IS_0_OVER_0.md`) |
| GRH Dirichlet L-functions via 0/0 | **g_χ(s) = \|L(s,χ)\|/\|L(1−s,χ̄)\| = 1 on the critical line for 8 Legendre symbols** (2026-08-17): `grh_dirichlet_0_over_0.py` extends the zeta 0/0 probe to Dirichlet L-functions. For each Legendre character (a/p) mod p ∈ {3,5,7,11,13,17,19,23}: Gauss sums verified \|G(χ)\| = √p to 4dp, root numbers \|ε(χ)\| = 1.000000, and g_χ = 1 on the critical line at t ∈ {10,25,50,100,150,200} with max deviation 0.0. The 0/0 at each zero of L(s,χ) has removable value \|ε(ρ)\| = 1, same as the zeta case. **HONEST WALL**: complete reduction (g_χ = 1 IS Re(ρ) = ½), not unconditional proof — GRH remains open (`data/grh_dirichlet_0_over_0_data.json`) |
| abc conjecture via 0/0 | **record quality 1.630, unit triple (1,1,1) is the 0/0 with removable value 1** (2026-08-17): `abc_conjecture_0_over_0.py` scans 38058 coprime triples (a,b,c=a+b) for quality q = log(c)/log(rad(abc)). Record: 2 + 6436341 = 6436343 with q = 1.6299 and rad = 15042. The 0/0 at the unit triple (1,1,1) — where log(1)/log(rad(1)) = 0/0 — has removable value 1 = the bound itself. The conjecture asserts q ≤ 1+ε for all but finitely many triples. **HONEST WALL**: finite verification of the classical record, not a proof of the conjecture (`data/abc_conjecture_0_over_0_data.json`) |
| Poincaré–Hopf via 0/0 | **index = removable value of V/\|V\| at zeros; S² Euler characteristic verified to 2** (2026-08-17): `poincare_hopf_0_over_0.py` computes the Poincaré–Hopf index of a vector field on S², T², and genus-2 surface. On S²: two zeros with index 1 each, sum = 2 = χ(S²) (match). On T²: constant field, χ = 0 (match). Removable value convergence: as contour radius shrinks from 0.1 to 0.0001 (3600 to 3600000 points), index converges to exactly 1 at every resolution. The 0/0 (V/\|V\| at zero) → winding number = removable value. **HONEST WALL**: computational verification of the Poincaré–Hopf theorem for specific surfaces (`data/poincare_hopf_0_over_0_data.json`) |
| Riemann–Roch via 0/0 | **l(D)−l(K−D)=0 at deg(D)=g−1 for genera 1–5; canonical divisor l(K)=g verified** (2026-08-17): `riemann_roch_0_over_0.py` verifies the Riemann–Roch theorem l(D)−l(K−D) = deg(D)−g+1 for genera 1–5 across all relevant degrees. At deg(D) = g−1 (half the canonical degree): the formula gives 0 = 0, the 0/0 form where both l(D) and l(K−D) may be nonzero but their difference is exactly 0. Canonical divisor: l(K) = g, deg(K) = 2g−2 (Serre duality). All identities hold for g=1 (elliptic), g=2 (bielliptic), g=3–5 (generic). **HONEST WALL**: Riemann–Roch is a proven theorem (not conjecture); the 0/0 framing highlights structural parallels (`data/riemann_roch_0_over_0_data.json`) |
| BSD via 0/0 | **L(1+ε,E) shrinks to 0 for rank 1, stabilizes for rank 0** (2026-08-17): `bsd_0_over_0.py` computes L(s,E) via truncated Euler product for 4 elliptic curves at ε = {0.5,0.2,0.1,0.05,0.01}. Rank 0 curves (y²=x³−x, y²=x³+1): L(1+ε) stabilizes (ratio > 0.8, value > 0.3). Rank 1 curve (y²=x³−25x, n=5 congruent): L(1+ε)→0 (ratio 0.69, value 0.11 at ε=0.01). The 0/0: L(s,E)/(s−1)^r has removable singularity at s=1; the removable value = leading coefficient encoding Sha and the regulator. **HONEST WALL**: BSD is a conjecture; this is a finite verification via Euler product, not a proof (`data/bsd_0_over_0_data.json`) |
| Argument principle via 0/0 | **(1/2πi)∮ζ′/ζ ds = Z inside 5 rectangles; 0/0 at each zero resolved by residue** (2026-08-17): `argument_principle_0_over_0.py` integrates ζ′(s)/ζ(s) around rectangles [0.1,0.9]×[im_min,im_max] and verifies the zero count matches the known zeros. All 5 test cases pass: 1, 2, 4, 0, and 8 zeros. At each zero ρ, ζ(ρ)=0 in the denominator of ζ′/ζ creates a 0/0; the residue (= multiplicity) is the removable value extracted by the residue theorem. **HONEST WALL**: the argument principle is a proven theorem; this is a computational verification (`data/argument_principle_0_over_0_data.json`) |
| Atiyah–Singer via 0/0 | **ind(D) = b₀−b₁+b₂ = χ(M) for S² and T²; Laplacian zero eigenvalues are the 0/0** (2026-08-17): `atiyah_singer_0_over_0.py` triangulates S² (162V,480E,320F) and T² (100V,300E,200F) via icosahedron subdivision and periodic grid. Euler characteristic: S² V−E+F=2, T² V−E+F=0. Laplacian zero-eigenvalue count gives Betti numbers b₀−b₁+b₂=χ(M) for both. The 0/0: the Laplacian eigenvalue 0 has multiplicity b_k; the index theorem says the alternating sum equals χ. **HONEST WALL**: Atiyah–Singer is a proven theorem; this is a combinatorial verification (`data/atiyah_singer_0_over_0_data.json`) |
| Gradient descent via 0/0 | **Hessian resolves saddle 0/0; Newton escapes in 1 step; 10D saddle confirmed** (2026-08-17): `gradient_descent_0_over_0.py` verifies the gradient descent 0/0 at saddle points. L(x,y)=x²−y²: ∇L=0 at origin, Hessian eigenvalues ±2 (saddle). GD 50 steps from (0.1,0.1) diverges along y-axis. Newton method (H⁻¹∇L) converges to minimum in 1 step. 10D saddle: 5 positive + 5 negative eigenvalues. The 0/0: at the saddle, ∇L=0 gives no update information; the Hessian eigenvalues are the removable value determining escape direction. **HONEST WALL**: pedagogical illustration of the 0/0 pattern, not a mathematical proof (`data/gradient_descent_0_over_0_data.json`) |
| Heat kernel trace via 0/0 | **Tr(e^{-tΔ}) → 1 as t → ∞; removable value = 1** (2026-08-17): `selberg_trace_0_over_0.py` verifies the heat kernel trace 0/0 on flat torus and sphere via analytical eigenvalues. Tr → 1.000000 at t=100 for both S² and T² (removable value = 1, the zero-mode contribution). At t=0: Tr → ∞ (Weyl singularity). The 0/0: lim_{t→∞} Tr(e^{-tΔ}) = 1, a removable value from the zero eigenvalue. **HONEST WALL**: computational verification of heat kernel properties, not a proof of the Selberg trace formula (`data/selberg_trace_0_over_0_data.json`) |
| Lefschetz fixed-point via 0/0 | **L(id) = χ(M); 0/0 at trace of f_* on H₁** (2026-08-17): `lefschetz_fixed_point_0_over_0.py` verifies the Lefschetz theorem on S² and T². S²: L(id)=2, Betti (1,0,1). T²: L(id)=0, Betti (1,2,1). Rotation on T²: trace on H₁ = 0 (the 0/0), removable value = local index. The 0/0 at the trace of f_* on each homology group determines fixed points via the alternating sum. **HONEST WALL**: computational verification on triangulated surfaces, not a proof (`data/lefschetz_fixed_point_0_over_0_data.json`) |
| Gauss-Bonnet via 0/0 | **int int K dA = 2 pi chi(M); 0/0 at zero-curvature points** (2026-08-17): `gauss_bonnet_0_over_0.py` verifies Gauss-Bonnet on S2, T2, and torus of revolution. S2: GB = 2pi*2 (chi=2, all K>0). T2: GB = 0 (chi=0, all vertices near K=0). Torus of revolution: GB = 0 (chi=0, mixed curvature). The 0/0: at K=0, angle defect = 2pi exactly -- the removable value. **HONEST WALL**: computational verification on triangulated surfaces, not a proof (`data/gauss_bonnet_0_over_0_data.json`) |
| Weyl's law via 0/0 | **N(lambda)/lambda^{d/2} -> C_weyl; 0/0 at lambda=0** (2026-08-17): `weyl_law_0_over_0.py` verifies Weyl's eigenvalue counting law on flat torus T2 and unit sphere S2. T2: relative error 0.07%. S2: relative error 0.78%. The 0/0: N(lambda)/lambda^{d/2} diverges at lambda=0 (0/0) but converges to C_weyl at lambda=inf. **HONEST WALL**: computational verification using known eigenvalues, not a proof of Weyl's law (`data/weyl_law_0_over_0_data.json`) |
| Central limit theorem via 0/0 | **phi(t) -> e^{-t^2/2}; 0/0 at (phi(t)-1)/t^2 = -1/2** (2026-08-17): `central_limit_theorem_0_over_0.py` verifies CLT for uniform, exponential, and Bernoulli. At t=0: (phi(t)-1)/t^2 = 0/0; removable value = -1/2. Uniform: ratio -0.4994, error 0.000061 at n=500. Exponential: ratio -0.4993, error 0.009. Bernoulli: ratio -0.4994, error 0.004. **HONEST WALL**: computational verification, not a proof of CLT (`data/central_limit_theorem_0_over_0_data.json`) |
| Banach fixed-point via 0/0 | **contraction -> unique fixed point; 0/0 at (T(x)-x)/(x-x*) = T'(x*)-1** (2026-08-17): `banach_fixed_point_0_over_0.py` verifies Banach on cos(x), Newton sqrt(2), and linear T(x)=0.5x+1. All converge. 0/0: cos removable = -1.6736, Newton removable = -1.0, linear removable = -0.5. **HONEST WALL**: computational verification, not a proof (`data/banach_fixed_point_0_over_0_data.json`) |
| Poisson summation via 0/0 | **theta functional eq: theta(s) = s^{-1/2} theta(1/s); xi(s) = xi(1-s); 0/0 at s=0** (2026-08-17): `poisson_summation_0_over_0.py` verifies the completed zeta function xi(s) = 0.5*s*(s-1)*pi^{-s/2}*Gamma(s/2)*Zeta(s). At s=0: 0*inf*(-0.5) = 0/0; removable value = 0.5. Near s=0: error 1.15e-08. Functional equation error: 3.89e-16. Theta equation error: 2.22e-16. **HONEST WALL**: numerical verification, not a proof (`data/poisson_summation_0_over_0_data.json`) |
| Rayleigh quotient via 0/0 | **R(x) = (x^T A x)/(x^T x); 0/0 at x=0; removable = eigenvalue** (2026-08-17): `rayleigh_quotient_0_over_0.py` verifies the Rayleigh quotient on 2x2 and 3x3 symmetric matrices. At x=0: 0/0. Along eigenvector v_i: R -> lambda_i (error < 1e-15). Along random direction: R -> v^T A v. Eigenvalue bounds hold: lambda_min <= R(x) <= lambda_max for all x. **HONEST WALL**: computation on specific matrices, not a proof (`data/rayleigh_quotient_0_over_0_data.json`) |
| Cauchy integral via 0/0 | **f(z)/(z-a) at z=a is 0/0; removable = f'(a)** (2026-08-17): `cauchy_integral_0_over_0.py` verifies removable singularities: sin(z)/z at 0 (removable=1), cos(z)/(z-pi/2) at pi/2 (removable=-1), (z-1)^2/(z-1) at 1 (removable=0), (e^z-1)/z at 0 (removable=1), (1-cos z)/z^2 at 0 (removable=0.5). Max error: 8.89e-05. **HONEST WALL**: numerical limit computation (`data/cauchy_integral_0_over_0_data.json`) |
| Noether/Landau via 0/0 | **M = tanh(M/T); 0/0 at T_c; removable = sqrt(3)** (2026-08-17): `noether_landau_0_over_0.py` verifies mean-field Ising. M(T_c)=0/0; amplitude -> sqrt(3) (err 1.85e-07). Above Tc: M=0. Below Tc: M>0. Free energy min at self-consistent M. **HONEST WALL**: mean-field theory, not finite-dim proof (`data/noether_landau_0_over_0_data.json`) |
| Euler-Maclaurin via 0/0 | **B(x) = x/(e^x-1); 0/0 at x=0; removable = 1** (2026-08-17): `euler_maclaurin_0_over_0.py` verifies Bernoulli generating function. B(x)->1 error 5e-14. Taylor coeffs match B_n. Sum-integral correction for f(x)=x^2 exact. **HONEST WALL**: numerical verification, not a proof of EM formula (`data/euler_maclaurin_0_over_0_data.json`) |
| Laplace method via 0/0 | **I(n)*sqrt(n) = sqrt(pi); 0/0 at n=0; removable = sqrt(pi)** (2026-08-17): `laplace_method_0_over_0.py` verifies Gaussian and quartic integrals. Gaussian: I(n)*sqrt(n)->sqrt(pi) (error 0). Quartic: I(n)*n^{1/4}->Gamma(1/4)/2. **HONEST WALL**: numerical verification (`data/laplace_method_0_over_0_data.json`) |
| Wallis product via 0/0 | **prod (2n)^2/((2n-1)(2n+1)) -> pi/2; 0/0 is 1^inf** (2026-08-17): `wallis_product_0_over_0.py` verifies the Wallis product for pi/2. Converges: error 3.93e-08 at N=10^7. Each factor -> 1 (error 2.50e-09). **HONEST WALL**: numerical verification, not a proof of Wallis (`data/wallis_product_0_over_0_data.json`) |
| Cesaro summation via 0/0 | **Grandi 1-1+1-1... Cesaro -> 1/2; geometric 0/0 at r=1** (2026-08-17): `cesaro_summation_0_over_0.py` verifies Cesaro summation. Grandi: Cesaro mean -> 0.5 (error 0). Geometric formula (1-r^{N+1})/(1-r) at r=1: 0/0, removable=N+1 (error 2.10e-05). **HONEST WALL**: numerical verification (`data/cesaro_summation_0_over_0_data.json`) |
| Fermat's little theorem via 0/0 | **(a^{p-1}-1)/(a-1) at a=1: 0/0, removable=p-1** (2026-08-17): `fermat_little_0_over_0.py` verifies Fermat via difference quotient. Removable value = p-1 for primes 2..47. Q(a) mod p = 0 for gcd(a,p)=1. Geometric sum identity holds. **HONEST WALL**: numerical verification (`data/fermat_little_0_over_0_data.json`) |
| Fundamental theorem of algebra via 0/0 | **f(z)/(z-z0)^k at z0: 0/0, removable=g(z0)** (2026-08-17): `fta_0_over_0.py` verifies polynomial root 0/0 via mpmath 80-digit precision. Simple, double, triple, complex roots all converge. Max error 6e-12. **HONEST WALL**: numerical evaluation, not a proof of FTA (`data/fta_0_over_0_data.json`) |
| Pythagorean theorem via 0/0 | **(a/c)^2+(b/c)^2 at c=0: 0/0, removable=1** (2026-08-17): `pythagorean_0_over_0.py` verifies Pythagorean triples (16 triples), continuous parameterization, degenerate limit. All ratios = 1.0 to machine precision. **HONEST WALL**: numerical evaluation (`data/pythagorean_0_over_0_data.json`) |
| Taylor's theorem via 0/0 | **R_n(x)/(x-a)^{n+1} at x=a: 0/0, removable=f^{(n+1)}(a)/(n+1)!** (2026-08-17): `taylor_remainder_0_over_0.py` verifies Taylor remainder 0/0 via mpmath 80-digit. Tests e^x, sin, cos, ln, mixed expansions. Max error 2e-13. **HONEST WALL**: numerical evaluation, not a proof of Taylor's theorem (`data/taylor_remainder_0_over_0_data.json`) |
| Fourier uncertainty via 0/0 | **sigma_x . sigma_xi / (1/4pi) at f=0: 0/0, removable=1 (Gaussian bound)** (2026-08-17): `fourier_uncertainty_0_over_0.py` verifies Heisenberg uncertainty bound. Gaussians achieve R=1 (equality); boxcars exceed; scaling f->0 keeps R constant (removable). **HONEST WALL**: numerical verification, not a proof (`data/fourier_uncertainty_0_over_0_data.json`) |
| Morse theory via 0/0 | **f/Q at critical point: 0/0, removable for min/max, not for saddle** (2026-08-17): `morse_theory_0_over_0.py` verifies Hessian classification, Euler characteristic, and 0/0 removability. Min/max ratios constant (removable); saddle direction-dependent (not removable); monkey saddle removable with value 0. **HONEST WALL**: numerical verification (`data/morse_theory_0_over_0_data.json`) |
| Brouwer fixed-point via 0/0 | **(f(x)-x)/(x-x*) at x*: 0/0, removable=Df(x*)-I** (2026-08-17): `brouwer_fixed_point_0_over_0.py` verifies fixed-point existence for contractions on D^2. Displacement ratio converges to Jacobian eigenvalues; spectral radius < 1 confirmed. **HONEST WALL**: numerical search, not topological proof (`data/brouwer_fixed_point_0_over_0_data.json`) |
| Stokes/de Rham via 0/0 | **boundary/surface ratio at empty boundary: 0/0, removable=1** (2026-08-17): `stokes_de_rham_0_over_0.py` verifies Stokes/Green's theorem for explicit forms on disk/rectangle. d^2=0 verified; angle form detects H^1(S^1). **HONEST WALL**: numerical verification (`data/stokes_de_rham_0_over_0_data.json`) |
| Sard's theorem via 0/0 | **image/domain ratio at critical point: 0/0, removable=0** (2026-08-17): `sard_theorem_0_over_0.py` verifies critical values form measure-zero sets. Finite 1D critical values; complex z^2/z^3 have isolated critical points; measure ratios vanish. **HONEST WALL**: numerical verification (`data/sard_theorem_0_over_0_data.json`) |
| KKT conditions via 0/0 | **mu/g at active constraint: 0/0, removable=shadow price** (2026-08-17): `kkt_conditions_0_over_0.py` verifies KKT for QP and barrier path. Complementary slackness mu*g=0; barrier mu_t*g_t -> 0; equality constraint shadow price. **HONEST WALL**: numerical verification (`data/kkt_conditions_0_over_0_data.json`) |
| Euler product via 0/0 | **(1-p^{-s})^{-1}/(1-q^{-s})^{-1} at s=0: 0/0, removable=ln(q)/ln(p)** (2026-08-17): `euler_product_0_over_0.py` verifies Euler product converges to zeta(s). Ratio P_N/zeta -> 1; local factor ratios -> ln(q)/ln(p). **HONEST WALL**: numerical verification (`data/euler_product_0_over_0_data.json`) |
| Picard's little theorem via 0/0 | **entire functions omitting values: e^z omits 0 only; sin(z)/z -> 1 at 0** (2026-08-17): `picard_little_0_over_0.py` verifies removable values at zeros of entire functions, e^z omits zero, cosh hits all values. **HONEST WALL**: numerical verification (`data/picard_little_0_over_0_data.json`) |
| Weil explicit formula via 0/0 | **-zeta'/zeta = sum_p log(p)/(p^s-1): zeros count the primes** (2026-08-17): `weil_explicit_0_over_0.py` verifies logarithmic derivative identity, explicit formula for psi(x) improves with zeros, prime counting corrected. **HONEST WALL**: numerical verification (`data/weil_explicit_0_over_0_data.json`) |
| Poincare recurrence via 0/0 | **eps*tau(eps) -> constant for irrational rotations; recurrence rate 0/0** (2026-08-17): `poincare_recurrence_0_over_0.py` verifies recurrence for golden ratio, periodic exact, equidistribution converges. **HONEST WALL**: numerical verification (`data/poincare_recurrence_0_over_0_data.json`) |
| Prime number theorem via 0/0 | **pi(x)*log(x)/x -> 1; pi(x)/Li(x) -> 1; pole 1/log(x) / 1/(x-1) -> 1 as x -> 1+** (2026-08-17): `prime_number_theorem_0_over_0.py` verifies PNT convergence, Li convergence, removable pole at x=1, Chebyshev bounds, error bounded. **HONEST WALL**: numerical verification, not a proof of PNT (`data/prime_number_theorem_0_over_0_data.json`) |
| Ising model via 0/0 | **phase transition at T_c = 2/log(1+sqrt(2)); magnetization drops, energy = -2*sqrt(2), susceptibility peaks** (2026-08-17): `ising_model_0_over_0.py` Monte Carlo simulation of 2D Ising model verifying order parameter, energy at T_c, and susceptibility divergence. **HONEST WALL**: Monte Carlo with finite-size effects (`data/ising_model_0_over_0_data.json`) |
| Khintchine (metric Diophantine) via 0/0 | **Dirichlet bound holds for all convergents; golden ratio q^2*|phi-p/q| -> 1/sqrt(5); Farey bound holds** (2026-08-17): `khintchine_0_over_0.py` verifies continued fraction convergents, golden ratio optimality, Dirichlet approximation, Farey bounds. **HONEST WALL**: numerical verification, not a proof of Khintchine's theorem (`data/khintchine_0_over_0_data.json`) |
| Schanuel conjecture via 0/0 | **e^a * e^b = e^{a+b} at a=-b (0/0 removable=1); Lindemann-Weierstrass verified; exp derivative 0/0** (2026-08-17): `schanuel_0_over_0.py` verifies exponential identity, algebraic independence evidence, transcendence degree heuristics. **HONEST WALL**: numerical verification, not a proof of Schanuel's conjecture (`data/schanuel_0_over_0_data.json`) |
| Shannon entropy via 0/0 | **0*log(0) = 0/0 removable value = 0; uniform max H=ln(n); MI 0/0 at perfect/deterministic; KL 0/0 at P=Q=0** (2026-08-17): `shannon_entropy_0_over_0.py` verifies information-theoretic 0/0 limits. **HONEST WALL**: numerical verification (`data/shannon_entropy_0_over_0_data.json`) |
| Bayes theorem via 0/0 | **posterior -> prior as P(D)->0 (0/0 removable = prior); likelihood ratio -> 1 as theta->0** (2026-08-17): `bayes_theorem_0_over_0.py` verifies Bayesian 0/0 limits. **HONEST WALL**: numerical verification (`data/bayes_theorem_0_over_0_data.json`) |
| Lorenz attractor via 0/0 | **Lyapunov exponent ~0.91 (log(1)/0 = 0/0); sum = -(sigma+1+beta); Hopf bifurcation at rho_H** (2026-08-17): `lorenz_attractor_0_over_0.py` verifies chaos, fixed points, bifurcation. **HONEST WALL**: numerical integration, not rigorous bounds (`data/lorenz_attractor_0_over_0_data.json`) |
| Boltzmann entropy via 0/0 | **S/ln(W)=1 (0/0 at W=1 removable=1); 0*ln(0)=0; mixing max at x=0.5; Gibbs pure=0** (2026-08-17): `boltzmann_entropy_0_over_0.py` verifies statistical entropy 0/0 limits. **HONEST WALL**: numerical verification (`data/boltzmann_entropy_0_over_0_data.json`) |
| Zeta functional equation via 0/0 | **zeta(0) = -1/2 via FE (0*inf = 0/0 removable = -1/2); FE verified; trivial zeros via sin(pi*s/2)=0** (2026-08-17): `zeta_functional_eq_0_over_0.py` verifies functional equation values, trivial zeros. **HONEST WALL**: naive truncated series, not mpmath (`data/zeta_functional_eq_0_over_0_data.json`) |
| Wigner semicircle via 0/0 | **GUE/GOE eigenvalue density = semicircle; rho/sqrt(4-lambda^2) = 0/0 at edge, removable = 1/(2*pi); spectral rigidity N(E)/E -> 1/pi** (2026-08-17): `wigner_semicircle_0_over_0.py` verifies semicircle, edge 0/0, Tracy-Widom scaling. **HONEST WALL**: Monte Carlo random matrices (`data/wigner_semicircle_0_over_0_data.json`) |
| Noether's theorem via 0/0 | **Conserved quantities from symmetry; dL/deps = 0/0 at eps=0 removable=0; momentum/energy conserved for free/harmonic/pendulum** (2026-08-17): `noether_theorem_0_over_0.py` verifies conservation laws and symmetry checks. **HONEST WALL**: numerical Euler-Lagrange integration (`data/noether_theorem_0_over_0_data.json`) |
| Spectral gap via 0/0 | **Gap closes at TFIM critical point h=1; Delta*L -> C (0*inf = 0/0); gap*L grows away from criticality** (2026-08-17): `spectral_gap_0_over_0.py` verifies critical scaling and gapped phases. **HONEST WALL**: exact diagonalization of small chains (`data/spectral_gap_0_over_0_data.json`) |
| Green's function via 0/0 | **1D/2D Green's functions; eigenfunction expansion converges; free-space G = -(1/(2*pi))*log(r); disk via method of images** (2026-08-17): `greens_function_0_over_0.py` verifies G(x,x), eigenfunction expansion, 2D formulas. **HONEST WALL**: numerical computation on simple domains (`data/greens_function_0_over_0_data.json`) |
| Mobius function via 0/0 | **mu(n) values correct; sum_{d|n} mu(d) = [n==1]; Mertens function; Dirichlet series 1/zeta(s) at s=1** (2026-08-17): `mobius_function_0_over_0.py` verifies Mobius values, Dirichlet inverse, PNT connection. **HONEST WALL**: numerical verification (`data/mobius_function_0_over_0_data.json`) |
| Saddle point via 0/0 | **Gaussian saddle converges; Stirling via Laplace; g'(x)/(x-x*) -> g''(x*) (0/0 removable); Watson's lemma** (2026-08-17): `saddle_point_0_over_0.py` verifies saddle point approximations. **HONEST WALL**: numerical verification of asymptotic methods (`data/saddle_point_0_over_0_data.json`) |
| Stirling's approximation via 0/0 | **n!/Stirling -> 1; correction 1/(12n); [ratio-1]*n -> 1/12 (0*inf = 0/0); Wallis product** (2026-08-17): `stirling_approx_0_over_0.py` verifies Stirling, corrections, Gamma function, Wallis. **HONEST WALL**: numerical verification (`data/stirling_approx_0_over_0_data.json`) |
| Mertens–ψ census (prime side) | **the zeros really DO count the primes, and both proven prime-side failures are invisible at every computable height** (2026-08-16): `mertens_psi_census.py` computes M(x), ψ(x), π(x) **exactly** to x = 10⁸ by a segmented sieve (μ verified against sympy for n ≤ 10⁶, zero mismatches; the classical Mertens table **M(10^k) = −1, 1, 2, −23, −48, 212, 1037, 1928** reproduced exactly, OEIS A084237) — the arithmetic side of the Littlewood/von Koch equivalences **RH ⟺ M(x) = O(x^{1/2+ε}) ⟺ ψ(x) = x + O(x^{1/2} log²x) ⟺ π(x) = Li(x) + O(x^{1/2} log x)**. Records over x ∈ [1000, 10⁸]: max |M(x)|/√x = **0.4722** at x = 2803 and it **never even reaches 0.5** (the points above 0.5 are only tiny x < 1000, e.g. x = 13); max |ψ(x)−x|/√x = **0.7770** at x = 1422; RH-normalized max |ψ(x)−x|/(√x log²x) = **0.0147** at x = 1422 (O(1) under RH); and **π(10^k) − Li(10^k) < 0 for every k = 1..8** (π lags Li at every height we can compute). The **explicit formula** ψ₀(x) = x − Σ_ρ x^ρ/ρ − log 2π − ½log(1−x⁻²), evaluated with the repo's OWN located zeros (653/4520/10142/22491 for T = 1005.43/5k/10k/20k; **653 matches the certified N(g₆₅₂)**), reproduces the sieve's exact ψ(x) with residuals that shrink as T grows (at x = 100, −0.169 for T = 1005.43 vs −0.006 for T = 20000). **THE TWO PROVEN-BUT-NEVER-SEEN FAILURES**: (a) the Mertens conjecture M(x) < √x is **PROVEN false** (Odlyzko–te Riele 1985; Pintz: counterexample < exp(1.59e40)) yet no explicit x is known and |M(x)| < √x holds for every x ≤ 10¹⁶ computed; (b) **π(x) > Li(x) is PROVEN to occur** (Skewes 1933/1955; first crossing < ~1.4e316 under RH, Bays–Hudson 2000) though π(x) < Li(x) at every computable height — both finite-failure theorems whose empirical evidence points the WRONG way. **RESOLUTION LIMIT**: RH needs the supremum over ALL x, while the best unconditional state is Korobov–Vinogradov ψ(x) = x + O(x·exp(−c (log x)^{3/5}/(log log x)^{1/5})) — an exponential-in-log-distance gap from the RH exponent. HONEST WALL: the arithmetic side confirms the S-side conclusion — numerical search is a counterexample engine, **RH remains open**; the proof, if it exists, is not a computation (`data/mertens_psi_census_data.json`) |
...
prime theory            prime_count_from_scratch.py  prime_gap_bridge.py  prime_time.py
                        ...
                        riemann_decimal_perspective.py  riemann_siegel_roots.py
                        riemann_siegel_certify.py  debruijn_newman_condensation.py
                        merger_scaling.py  connes_letter.py  connes_dirac.py
                        four_point_lattice.py  zeta_lattice_alignment.py
                          zeta_direct_probe.py  zeta_interlacing_certify.py
                           riemann_siegel_ordinate.py  s_function_census.py
                            mertens_psi_census.py  mertens_sublinear_census.py
                            mertens_explicit_height.py  mertens_psi_height.py
                             body_fold_symmetry.py  zeta_zero_spectral_match.py
                            grh_dirichlet_0_over_0.py  abc_conjecture_0_over_0.py
                            poincare_hopf_0_over_0.py  riemann_roch_0_over_0.py
...
| O(1) spatial search T67 (`decentral_net_t67.py`) | SUPPORTED (measured): DecentralNet(use_index=True) replaces the n×n distance matrix with a numpy uniform grid (dim ≤ 3, Chebyshev ring-expansion proven exact) or scipy cKDTree (dim ≥ 4). Indexed flow is **bit-identical** to exact all-pairs (grid 2D n=2000 + tree 64D n=500; spacing/predict equal; grid kNN == brute force on 3 seeds × 1D/2D/3D). Scaling exponent 1.11 (indexed) vs 1.92 (exact) in 2D — flow is now ~linear; n=100,000 flows at 10.35 s/step where the all-pairs D would need 160 GB, and 10,000 real top-1M domains at 128-D run at 4.12 s/step (102 GB all-pairs). Honest walls: k-d trees still degenerate in high dim on dense data (high-dim ceiling ~10⁴), the grid is numpy-only with a lazy scipy fallback to exact all-pairs on ImportError, and internet-scale flow is still one machine |
...
**T72** then flowed the *entire* real internet net (1,914,915 sites — union of Cisco Umbrella + Majestic top-1M, PCA(2) of real 128-D embeddings): **~449 s/step** where all-pairs D would be 58,670 GB; consensus spacing rises across the population; a 20% kill (382,983 sites) recovers **+7.8%** spacing after heal(1) across 1.53M survivors. Honest wall: 128-D native flow stays near 10⁴ (crease #22; measured 19.1 s/step on 10k real sites, all-pairs D 102 GB), and this is still one machine. The T72 claim-set (T1 flow the whole internet, T2 self-healing at full population, T3 the measured high-dim wall) is pinned by `test_decentral_net_t72_supported`. See `experiments/decentral_net_t67.py`, `experiments/decentral_net_t72.py`, `data/decentral_net_t67_data.json`, `data/decentral_net_t72_data.json`.

**T55g/T55i/T55j** — the *whole-internet pipeline* that feeds T72 — are now verdict-pinned too: `decentral_net_internet.py` bulk-loads the real top-1M sites (I1), routes them by nearest-centroid over real internet geometry (I2, google.com→gooogle.com etc.), survives a 20% outage with no repair unit (I3, 800k of 1M stay routed), and flows a real 1000-site slice at ~862 ms/step with spacing recovery after local heal (I4); `decentral_net_union.py` merges + dedupes two independent top-1M lists to **1,914,915 unique widely-used sites** (U1), shows holding is ~2 KB/site so capacity is not the wall (U2, 3.92 GB), routes across the merged lists (U3), survives the outage (U4), and persists a checkpoint that reloads bit-identically (U5); `decentral_net_anomaly.py` validates the geometry as a name-space anomaly detector against the real legit population — 90% of DGA-shape random strings and 32% of known-bad fall below the legit p5 threshold (novelty axis, A1) while 18% of known-bad sit above the legit median (near-miss impersonation, A2), but the blocklist overlap proves name geometry is **necessary-but-not-sufficient** for separating bad from legit (A3 — the multivariate observation bank is the unbuilt next layer). All four ship claim/verdict JSONs pinned by `tests/test_solvable_theorems.py`.
